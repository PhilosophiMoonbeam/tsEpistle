...
OKF authority is implicit for every Markdown page create, read, update, move, restore, import, export, and download. Wiki source remains the authoritative page and revision record; normal human, API, and Agent reads expose the authority record and the projection matched to that source revision. Use raw `wiki_get_page_okf` only when an exact canonical interchange document is required, not for an ordinary page read. Its immutable resource URI is `wiki://pages/{pageId}/versions/{version|current}/revisions/{sourceRevision}/okf`.

Markdown storage uses `{locale}/{path}.md`; the reserved page paths `index` and `log` use `{locale}/index.concept.md` and `{locale}/log.concept.md`. Storage ingress classifies OKF, legacy, plain, and invalid documents, rejects malformed claimed OKF rather than treating it as legacy, verifies canonical OKF paths, strips imported verification claims at the server-owned mutation boundary, and preserves the source bytes and hash. Canonical Markdown egress and direct downloads emit deterministic OKF; non-Markdown pages retain their existing serializers. Administration → Storage shows target status and the latest import/export operation.

The human editor's **Knowledge / OKF** panel edits authority metadata and displays trust and the revision-matched projection. A missing or invalid authority record can be reset to a stable `Reference` record before saving. Projection lifecycle and maintenance status are operational signals only. Utility enrichment may fill only declared gaps on the current public revision; private pages fail closed, and projection trust/lifecycle fields remain retrieval signals, never permissions or evidence.

The MCP endpoint is built directly on the official Model Context Protocol TypeScript SDK v2, supports the current `2026-07-28` protocol, and retains tested legacy negotiation. Deployment, security, projection, storage, and interchange details are in [Agent deployment and operations](docs/agents-deployment.md).
