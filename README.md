...
> **Recovery contract:** Use the [Compose upgrade command](deploy/compose/README.md#upgrades) for risk-selected local redeployments. Routine code-only replacements preserve existing state without creating a new backup. For persistent-state changes, capture PostgreSQL and all of `/wiki/data` together. After a migration commits, restore both from the same pre-upgrade point before starting the old image; starting old code against a newer schema is unsafe.
