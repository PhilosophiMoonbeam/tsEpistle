...
Citation validation is clause-local and keeps the existing 0.6 lexical threshold, but support must come from one intact source sentence, list item, table row, or presentation unit within the exact delivered citation scope. Section identity follows the citation label's complete heading ancestry and fails closed when repeated headings remain ambiguous; it never guesses from citation position or a generated anchor slug. Parent sections retain descendant content, including nested details whose exact entering heading stack is restored on close, while child citations retain their identifying heading ancestry without borrowing parent or sibling body facts. Standalone headings and list presentation may establish that a named item is listed, but ancestry identifies the local subject without importing body predicates. Named and short identifiers are compared source-locally without capitalization bypasses. Numeric and mixed-digit claims bind to faithful source assignment segments while allowing an intact listed assignment to be summarized alone; polarity and body temporal/restriction qualifiers retain their local attachment. A heading qualifier such as `Current` may identify its child fact, but it does not erase or replace a qualifier required by the source body. Source units remain validation evidence regardless of character length.
