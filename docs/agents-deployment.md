...
The core Agent deployment, topology, migration, and runtime guidance in this document describes historical release candidate `1d1e25a7c131880fc963084fa993b73a19d579bc`; keep those operational baseline and threat-model conclusions bound to that exact historical source revision. Under the manifest-driven security gate (`docs/security/review-attestations.json`), release eligibility is governed by immutable structured review records rather than the monolithic markdown `Covered source` row. The later search foundation closeout reflects an intentionally uncommitted working-tree delta with content fingerprint `3adffcba7255a3f88ec490ceffefe6f826e9508a` and is captured as staged evidence only (`kind: "working-tree-audit"`); it is explicitly not deployed, not release-eligible, and not globally threat-attested.
...
The unified `operations.search` cutover and utility boundary are implemented, but this working tree is intentionally uncommitted and not deployed. The foundation record retains focused implementation proof, four explicitly enabled real PostgreSQL suites with all 21 cases / 61 assertions passing (`knowledge-search` 4/10, `utility-admission` 3/5, `search-foundation` 12/37, and `search-graph-rank` 2/9), server types, eight scoped files after two fixture corrections, a passed isolated PostgreSQL 17.10 / pg_trgm 1.6 20,000-page benchmark report (30 iterations, 5 warmups, rebuild 25,772.876510 ms, p95 query latencies within thresholds, zero violations), migrated MCP-focused tests (7/7), native reader/admin light/dark accessibility (zero WCAG 2 A/AA and 2.1 AA violations) and continuation evidence (25 results loaded as 20 + 5 with scope preserved, mutation continuation returning surviving 4 tail identities with no duplicates and 409 mismatch rejection), native browser search/tree/preview/focus observations, and least-privilege MCP resource/revocation proof. The full isolated test suite passed **472/472** files, and static gates (dependency policy, 800 licenses, lint, shared/client/server typechecks, OpenAPI compatibility/lint, production placeholders, Agent release gate, and Vite build/bundle budgets) passed. The preview build used explicit supported `WIKI_BUILD_REVISION` content fingerprint **`3adffcba7255a3f88ec490ceffefe6f826e9508a`**, not a Git commit. Maintained instance content and settings at `https://agents8c48g.tail41a24a.ts.net:10443/` were verified unchanged against initial fingerprints with zero repository bind mounts and no production deployment, and disposable audit infrastructure (preview process, disposable PostgreSQL container, sensitive snapshot and configuration directory, and browser/database handles) was stopped and removed; these operational assertions are directly retained in [`search-foundation-live-2026-09-07.json`](benchmarks/search-foundation-live-2026-09-07.json) (`source.deployedToMaintainedInstance=false`, `maintainedAuthoritativeState.pagesUnchanged=true`, `pageHistoryUnchanged=true`, `settingsUnchanged=true`, `repositoryBindMounts=0`, and `cleanup` flags all true with empty `remaining=[]`). The fixture-augmented benchmark is not a live-model result; actual configured-utility reports (`gpt-5.6-luna`, profile `3b2c09ac-a6e6-4463-904a-dd51a7ee00ac`) remain separately retained and retain the held-out semantic miss on `restore the ultraviolet verification code`. Repository-wide release attestation (`ci:static` and `bun server/scripts/check-threat-model.ts --release`) remains blocked by the active review record in `docs/security/review-attestations.json` (the legacy maintainer baseline `legacy-baseline-55f30709` for `55f30709cd4b8de0ec1fd498cac7174f1cacd084` with `independent: false`, `releaseEligible: false`, and blocking finding `SEC-EXT-001`). The monolithic markdown `Covered source` row has been superseded by this manifest-driven gate, and the uncommitted search audit (`search-foundation-2026-09-07.json`) is staged working-tree evidence (`releaseEligible: false`) rather than a releasable attestation.
...
The continuation store remains process-local by design. Confirm worker affinity before exposing continuation across replicas. A missing affinity is an operational limitation to resolve deliberately, not a reason to bypass owner/query binding, expiry, or current-access reauthorization.
...
Official beta and production release publications are gated by the executable security check (`bun server/scripts/check-threat-model.ts --release`). Release qualification is strictly separated from working-tree development:

1. **Manifest-driven attestation and external trust root:**
   Releases are governed by `docs/security/review-attestations.json`, which references immutable review records in `docs/security/review-attestations/<id>.json`. In-repository booleans (`independent: true`, `releaseEligible: true`) alone do NOT authorize deployment or release: official release requires an independent, release-eligible `source-review` record cryptographically certified with a detached Ed25519 signature verified against an external trusted-key set (`THREAT_REVIEW_TRUSTED_KEYS_JSON`). Every `source-review` record asserting `reviewer.independent: true` OR `releaseEligible: true`—whether active or historical—strictly requires a structurally valid detached Ed25519 signature verified against an externally trusted key. Trusted keys must never be stored in this repository; in CI, the public verification keys are provisioned via the public repository variable `vars.THREAT_REVIEW_TRUSTED_KEYS_JSON`, ensuring PR-safe availability across PR static checks, quality, and release gates.
2. **Releasable source reviews vs. staged working-tree evidence:**
   - A releasable attestation must be a committed `kind: "source-review"` record with `repository: "PhilosophiMoonbeam/tsEpistle"`, `reviewer.independent: true`, `releaseEligible: true`, zero blocking findings, an exact canonical `coveredTreeDigest` match against HEAD, and a valid detached Ed25519 signature matching the canonical payload (`tsepistle-threat-review-attestation-v1\0` followed by recursively key-sorted compact JSON without signature) verified against an active trusted key whose identity strictly matches `reviewer.identity`.
   - Working-tree audits (such as `search-foundation-2026-09-07.json` for content fingerprint `3adffcba7255a3f88ec490ceffefe6f826e9508a` based on commit `d3d5a63326c056bb847fdb2552be0a1cf1eb7020`) are staged evidence only (`reviewer.independent: false`, `releaseEligible: false`, unsigned, `fingerprintAlgorithm: "tsepistle-isolated-preview-v1"`). Working-tree fingerprints never qualify as release attestations and cannot bypass release gating.
   - The `publish-canary` job publishes a non-production, pre-release validation image from `main`; it is gated by the ordinary manifest/digest check in `ci:static` plus architecture-specific image tests, not by `--release`, and it makes no independent-review or release-eligibility claim. Operators must not promote or treat a canary image as a beta or production release; those publication jobs remain subject to the strict release gate.
3. **Exact non-circular release procedure:**
   - **Freeze and commit source:** Commit all changes within the canonical security boundary (the entire `dev/` prefix, `server/`, `client/`, `shared/`, `deploy/`, `patches/`, `.github/workflows/`, `.github/actions/`, root release/config manifests, `tsconfig*.json`, and root packaging, repository attribute, and legal compliance files `.dockerignore`, `.gitattributes`, `LICENSE`, `NOTICE`; these root files are essential to the boundary because `.dockerignore` controls the container build context boundary, `.gitattributes` governs release archive export filtering and line endings for release bundles, and `LICENSE`/`NOTICE` establish mandatory distribution licensing terms).
   - **Calculate digests:** Compute canonical boundary tree and threat-model digests with `bun server/scripts/check-threat-model.ts --digest <revision>`.
   - **Create structured review record:** Write an immutable `source-review` record in `docs/security/review-attestations/<id>.json` declaring `repository: "PhilosophiMoonbeam/tsEpistle"`, `reviewer.reviewedAt` in exact UTC calendar date format `YYYY-MM-DD`, and resolving or accepting all findings structurally.
   - **Sign detached attestation:** Sign the canonical payload (`tsepistle-threat-review-attestation-v1\0` concatenated with compact JSON with recursively sorted keys, omitting `signature`) with the independent reviewer's Ed25519 private key, attaching the resulting signature object (`{ algorithm: "ed25519", keyId, value }`).
   - **Update manifest:** Point `activeReviewId` in `docs/security/review-attestations.json` to the new record and include it in `records`.
   - **Commit attestation docs:** Commit the documentation changes under `docs/security/`. Because documentation (`docs/`) is excluded from the canonical security boundary, this commit does not alter the `coveredTreeDigest`.
   - **Execute release gate:** Run `bun server/scripts/check-threat-model.ts --release` in a clean working tree with `THREAT_REVIEW_TRUSTED_KEYS_JSON` configured in the environment (provided via the public repository variable `vars.THREAT_REVIEW_TRUSTED_KEYS_JSON` in CI). The release gate rejects any dirty repository path (with the sole canonical exception of the non-symlink regular file `server/.build-metadata.json` containing exact `revision` and `date` strings matching current HEAD and a valid canonical ISO timestamp in exact JavaScript `Date.toISOString()` canonical form `YYYY-MM-DDTHH:mm:ss.sssZ`; any malformed JSON, symlink, or revision mismatch fails closed), requires an exact covered-tree digest match against HEAD, verifies zero blocking findings, checks schema assertions, and verifies the Ed25519 signature against the external trusted-key set for all independent or release-eligible source reviews, strictly enforcing signer identity binding. If the trusted keys are absent, untrusted, or invalid, release is blocked.
## Incident runbook
