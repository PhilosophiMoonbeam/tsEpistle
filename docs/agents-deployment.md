...
The **Catch Up** starter is a normal read-only request. Its recent-page and page-read results are passed as bounded whole packets: the engine either admits a complete projected result that fits its prospective provider context or records that result as omitted; it does not truncate a source packet into a misleading partial source. A result that completed at the Wiki action layer but cannot fit provider context is returned to synthesis with `status: "omitted"` and `reason: "tool_result_capacity"`. Calls after capacity is reached are not invoked and receive `status: "not_executed"` with the same reason. Each disposition carries the explicit action-call ID and a paired durable activity.

The durable event envelope is `{ id, runId, sequence, attempt, schemaVersion: 1, type, data, createdAt }`; PostgreSQL also stores the canonical `dataSha256`. Every model-requested action has exactly one `tool.started` event and exactly one terminal event. `tool.started` records `actionCallId`, `actionName`, display `title`, `risk`, `turn`, and, when canonical input was recorded, `input`. A delivered action uses `tool.completed` with `actionCallId`, `actionName`, and its bounded serialized `result` (plus optional `summary`, `cacheHit`, and `reusedActionCallId`). An action that executed but whose result cannot fit provider context still uses `tool.completed` and retains that bounded result for audit, adding exactly `contextExclusion: { status: "omitted", reason: "tool_result_capacity" }`; the provider receives only the bounded omission envelope. A call skipped after capacity is reached uses `tool.notExecuted` with `actionCallId`, `actionName`, and exactly `contextExclusion: { status: "not_executed", reason: "tool_result_capacity" }`, with no `result` and no Wiki invocation. A genuine action failure uses `tool.failed` with its `actionCallId`, `actionName`, and allowlisted `errorCode`, never a capacity exclusion. The activity projection states are therefore `complete`, `omitted`, `not_executed`, or `failed`; omitted and not-executed are capacity dispositions, not failures.

`contextExclusion` is a strict two-field object: `status` is only `omitted` or `not_executed`, and `reason` is only `tool_result_capacity`. A not-executed event cannot carry a result; a completed event cannot claim `not_executed`; and a failed event cannot carry a context exclusion. Missing starts, duplicate terminal events, mismatched action names, or any other shape violation fails closed as `AGENT_EVENT_CORRUPT`. The provider-facing envelopes are likewise explicit: `{ status: "omitted", reason: "tool_result_capacity", contextExclusion: { status: "omitted", reason: "tool_result_capacity" }, actionCallId, actionName, message }` for an executed-but-undelivered result, and `{ status: "not_executed", reason: "tool_result_capacity", contextExclusion: { status: "not_executed", reason: "tool_result_capacity" }, actionCallId, actionName, message }` for a capacity-skipped call.

Only delivered, successful page reads enter the citation/evidence registry. Recent-page/list/discovery results remain candidate metadata; an omitted read is retained for diagnostics but is not evidence, and neither omitted, not-executed, nor failed output may be cited. `evidence.provenance` records bounded `accepted`, `issues`, `retrievals`, `claims`, and `finalCitationIds`; a rejected draft records its issues and may receive a bounded tool-free correction turn, while an accepted final answer must cite the delivered reads it uses. Diagnostics reconstruct the complete start/terminal activity timeline, expose capacity exclusions separately from `tool.failed`, and may retain bounded canonical action inputs/results for audit; they do not export provider-state ciphertext, hidden reasoning, credentials, prompts, or raw provider errors.

When a result is omitted or a later call is not executed because of context capacity, the engine adds a bounded coverage notice, switches permanently to tool-free synthesis, and preserves monotonic collection/accounting. It may still return a useful assistant message grounded in the delivered evidence, but it must disclose the coverage limitation and never claim complete coverage, cite an omitted source, fabricate a missing finding, or automatically repeat the same collection. The engine returns `contextLimit: { reason: "tool_result_capacity", omittedActionCallIds }`; runtime persists the assistant message with status `complete` while marking the run `partial`, emits a blocked completion assessment with `AGENT_CONTEXT_TOO_LARGE` and `retryable: false`, and does not schedule an automatic repeat. This capacity path has no genuine action failure. If all requested evidence fits and no capacity omission occurs, Catch Up retains its ordinary completion assessment rather than being forced to `partial` or `blocked`.
...
### Current deployed Catch Up evidence and historical framing

The deployed live **Catch Up** request directly exercised the result-capacity path. It recorded **11 durable activities: 3 delivered, 1 omitted, 7 not executed, and 0 failed**. The final `evidence.provenance` was accepted and cited only delivered evidence; the assistant message was persisted with status `complete`, while the run was terminalized as `partial` and its completion assessment was `outcome: "blocked"` with `AGENT_CONTEXT_TOO_LARGE` and `retryable: false`. The seven activities previously described as failures are capacity-skipped calls, and the one omitted activity is an executed result withheld from provider context; neither is a genuine provider or Wiki-action failure. The current Catch Up root cause is therefore known and reproduced as bounded provider-result capacity, with no pending or automatic rerun for this non-retryable condition.

The earlier third-turn provider-request/continuation report remains historical context, but it is a separate incident from the deployed capacity result and must not be used to label this run a provider failure. For current operations, capacity is established by the explicit `tool.completed`/`tool.notExecuted` dispositions and blocked partial assessment; a genuine provider or action failure requires its own `tool.failed` event and allowlisted failure telemetry.
