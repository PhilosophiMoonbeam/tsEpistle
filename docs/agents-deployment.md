...
The core Agent deployment, topology, migration, and runtime guidance in this document describes historical release candidate `1d1e25a7c131880fc963084fa993b73a19d579bc`; keep those operational baseline and threat-model conclusions bound to that exact historical source revision. The current governing release policy is the maintainer-owned `schemaVersion: 2` manifest and source-review gate in `docs/security/review-attestations.json`. The later search foundation closeout reflects an intentionally uncommitted working-tree delta with content fingerprint `3adffcba7255a3f88ec490ceffefe6f826e9508a` and is captured as staged evidence only (`kind: "working-tree-audit"`); it is explicitly not deployed, not release-eligible, and cannot supersede the current source review. Any older release-policy wording in this historical operations record is superseded by the schema-2 maintainer policy below.
...
A provider stream is considered complete only after genuine EOF. If reading, accepting, or normalizing a chunk fails before EOF, the engine awaits exactly one cancellation with a fixed safe reason and then releases the reader lock even when cancellation rejects; it preserves the normalized `provider_stream`/`provider_response` failure and keeps unsettled exposure conservative. Normal EOF does not cancel and is the only stream path that completes usage reconciliation.
...
Provider streams still complete only after genuine EOF. A pre-EOF read, acceptance, or normalization failure receives exactly one cancellation attempt and keeps the normalized failure plus conservative unsettled exposure; normal EOF does not cancel and is the only stream path that completes usage reconciliation.
...
The token-accounting deployment is a coordinated writer cutover: **drain workers → migrate → deploy all nodes → restart**, with no mixed old/new writers.
