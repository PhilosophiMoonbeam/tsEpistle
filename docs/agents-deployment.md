...
This document separates the current normative runtime contract and mode-selected deployment procedures below from the historical deployment record that follows. `AGENTS.md` selects **Development Sprint** by default and activates **Enterprise Release** only for an explicitly named milestone. The historical record retains the release-candidate baseline `1d1e25a7c131880fc963084fa993b73a19d579bc`, its operational conclusions, the maintainer-owned `schemaVersion: 2` source-review policy, and the later search-foundation working-tree evidence (`3adffcba7255a3f88ec490ceffefe6f826e9508a`); that evidence is not deployed or release-eligible. Do not treat historical instructions as authority when they conflict with the current mode contract.


## Current normative runtime and recovery contract

This section is the operator authority for runtime behavior and safe operating boundaries. It is not itself a build, deployment, live-provider run, or release-attestation result. In Development Sprint, retain a truthful focused smoke result for the changed behavior. Enterprise Release retains its comprehensive acceptance evidence. A health response alone is insufficient in either mode.
...
- Persisted provider-native continuation is optional, dialect-specific state attached only to an accepted final root answer. Planner/subagent output, failed runs, and cancelled runs do not persist it. The outer envelope is exactly `{ schemaVersion: 1, continuationDialect, thoughtBlocks }`; each bounded block is `{ data, encrypted: true, signature? }` after dialect-specific validation. OpenAI Responses/OpenResponses retain bounded provider reasoning blocks and may retain a validated `rs_…` result identifier. OpenAI Chat and Anthropic Messages likewise retain bounded provider blocks marked encrypted; rejected-answer reasoning is not carried into their evidence-repair turns.
- Gemini Interactions retains validated ordered native steps, signatures, annotations, and the current run's intervening function results/corrections. Its accepted answer can therefore preserve earlier grounding across host-tool turns without duplicating older canonical conversation history. The inner payload accepts historical step arrays and `{ steps, assistantStepStart }`, whose suffix must match the visible final assistant answer. Raw Search Suggestion HTML is removed, not the grounded text or native context. Gemini's `encrypted: true` marker denotes a validated format, not application encryption. PostgreSQL stores these bytes with a SHA-256 integrity check; a local database operator can inspect locally encoded content.
- Replay reads the canonical transcript and attaches that state only when the current run matches the exact origin owner, session, provider-profile version, transport kind, model, and capability revision. Current versioned state must pass its stored integrity hash, schema, expected dialect, encrypted-block marker, block-count, and byte-limit checks. An incompatible origin, dialect, or unrecognized legacy shape omits the state and falls back to the canonical transcript; matched versioned or integrity-bearing state that fails validation fails closed as `AGENT_PROVIDER_STATE_CORRUPT`. A bounded provider result or reasoning-item ID is replay metadata, never resume authority. Supported persisted dialects are OpenAI Responses, OpenResponses, Gemini Interactions, OpenAI Chat, and Anthropic Messages; legacy text Completions has no persisted continuation dialect. Never remap continuation state to a different owner, session, profile version, transport, model, or capability.

- Provider-native continuation admission is atomic at the existing continuation-state byte budget. The smaller text, argument, and transport-fragment bounds govern those fragment classes; they do not impose an additional per-block cap on a complete validated native continuation item. This distinction does not raise the continuation cap, relax genuine-EOF validation, or change accounting.
- A Gemini Interactions request with no declared tools explicitly sends `tool_choice: "none"`, including synthesis after compaction. Answer-only turns also state that no Wiki actions are admitted and that prior calls are history rather than current permission. Declared functions and enabled Google Search retain their existing choice policy. Unexpected provider calls still fail closed without executing an action.
- Provider connection conformance checks cancellation before dispatch, then performs the declared native or prompt action round trip with a fresh receipt available only in the tool result. Its final request has no admitted actions; passing requires an exact receipt-bearing answer, no unexpected calls, and declared usage after complete output. A generic provider failure on a pre-aborted request is not cancellation support.
...
### Application-managed context compaction

- tsEpistle compacts older complete conversation groups and completed tool batches; it does not silently drop conversation history to make an overflowing request fit. The current user request, unresolved tool batch, and newest completed active batch stay intact. Original messages and personal-memory snapshots are not rewritten.
- Summarization uses the run's pinned **main Agent model**, not the utility model or provider-native compression. The call has no actions, Google Search, or attachment payloads. Its input contains visible conversation/tool text, not hidden reasoning, signatures, or raw Search Suggestion HTML. Summary prose is explicitly untrusted context, never new instructions, citation evidence, approval, or resume authority.
- Policy scales with the approved profile window and output reserve. For a 128,000-token profile with an 8,192-token answer ceiling, the trigger is **96,000 conservative exposure units**, the post-compaction target is **76,800**, the summary aims for **2,000 narrative tokens**, and its provider output ceiling is **4,000 tokens**. JSON-encoded summary content is capped at **16,000 UTF-8 bytes**, with smaller windows receiving smaller caps. Exposure uses conservative serialized request bytes; it is not a measured input-token count. A fixed 135,000 threshold would exceed this profile's window.
- Each fold must finish at genuine EOF with `stop`, contain no action or search output, stay within its encoded bound, and strictly reduce serialized context. Empty, partial, oversized, or nonshrinking output is rejected without truncating the original. An unchanged failed source is not summarized repeatedly in the same run. If the complete unchanged request still fits, it may proceed; otherwise the run fails closed.
- Historical summaries are complete, integrity-checked `model.turn` checkpoints with `outcome: "context_compacted"`. Reuse requires exact owner/session/provider binding, canonical source-prefix hash and watermark, and an unexpired inherited deadline. Lease-fenced persistence and source revalidation finish before replacement. Active tool-context summaries remain run-local and are not embedded in accepted Gemini native continuation.
- Before summarizing, admission reserves the planned folds **and** bounded follow-on request together. Every accepted or rejected summary call contributes ordinary main-model token/cost usage exactly once; checkpoint reuse does not re-charge it. Compaction does not reset goal accounting or grant more actions or continuations. Preflight remains provider-call-free.
- Derived context inherits the earliest grounded-source expiry, rather than receiving a fresh two-year lifetime. Dispatch and publication recheck freshness; completed usage survives rejection of an expired answer. Maintenance removes expired summary/draft content even from running or awaiting-approval runs, while preserving accounting and unrelated messages.

### Opt-in Gemini Google Search

- **Web search** is off by default. A session-versioned, owner-scoped update enables it only for an available Gemini Interactions profile. Consent changes lock the session against run admission and are rejected during active work. Explicit profile changes clear consent. Run admission snapshots consent, binds true consent into profile/admission hashes, and preserves legacy hashes for false/missing consent. Goal continuations cannot silently change the original authority.
- Only root text dispatches receive the native `google_search` tool; utility, planner, subagent, and media requests do not. Mixed native/function requests use `tool_choice: "validated"`. Turning search off prohibits new native search calls but permits follow-ups to use and cite retained context. It does not delete saved answers or force a context reset; implicit cache hits remain provider-controlled.
- Native web citations remain distinct from Wiki evidence and confer no Wiki authority. Original native UTF-8 byte offsets remain in replay state; displayed citation ranges are validated and converted to JavaScript UTF-16 indices. A response allows at most 32 credential-free HTTPS citations.
- Search Suggestion HTML is transient: at most eight strings of 32,768 characters, with the complete event limited to 128 KiB UTF-8. Owner/run-fenced local or chunked PostgreSQL notifications feed the id-less `google_search.suggestions` SSE event. The widget is held only in client memory and rendered in an opaque-origin, scriptless, restrictive-CSP iframe. It is not saved in messages, durable events, or native continuation, and reconnect/reload does not replay it.
- Answer text, citation metadata, and sanitized native context remain in normal private chat history. Responses using search or retained grounding have a two-year maximum retention, including interrupted output and rejected diagnostic drafts; ordinary earlier retention/deletion still applies. Expiry removes that content/context, not unrelated messages or token accounting.
- Search queries may incur Google charges beyond the displayed token-cost estimate. No native per-request query-cost ceiling is promised. See [Google Search grounding](https://ai.google.dev/gemini-api/docs/google-search) and its [terms](https://ai.google.dev/gemini-api/terms#grounding-with-google-search).
...
- Invalid terminal grounding presentation metadata is deferred until the bounded stream reaches genuine EOF, then fails after settling the valid receipt. A subsequent stream failure still retains conservative exposure; a terminal-looking chunk alone is not EOF.
...
On final no-tool turns, an explicit provider `finishReason: length` is retained in `model.turn` and ends that inference attempt without automatic correction or continuation. A root answer publishes only evidence-validated partial text plus an output-limit notice, omits provider continuation state, and finishes as a blocked partial run with nonretryable `AGENT_OUTPUT_LIMITED`. An invalid or empty fragment publishes only the notice. Limited planner output is rejected through the ordinary no-plan fallback; limited child output becomes a host-owned blocked research packet with no claims. Neither structured channel receives prose or credits unfinished provider JSON. Follow-up or goal resume requires an explicit user action; limits and reasoning effort are unchanged.
...
The usage contract keeps independent nonnegative safe `inputTokens`, `outputTokens`, and provider `totalTokens`, with `totalTokens >= inputTokens + outputTokens`; residual tokens remain deliberately unclassified. For OpenAI Responses/Chat/OpenResponses and Anthropic, Ax's uncached prompt count is combined with any reported cache-read and cache-creation counts **once** to form full input; Gemini already includes cached tokens in its input. Full input and the independent total remain the quota basis. Provider-reported cost uses configured directional rates and the maximum rate for residual tokens, with safe integer arithmetic; recognized official Anthropic and GPT-5.6+ OpenAI profiles additionally use the conservative 1.25× cache-write input rate, including for reserved exposure, without assuming a read discount. Nullable `cachedInputTokensReported` and `cacheCreationInputTokensReported` are diagnostics: null is unavailable through the adapter, not zero or a cache miss. Pinned Ax omits OpenAI zero cache counters even when the wire reported zero. An explicitly permitted missing-usage fallback is an admitted estimate charged at the full exposure; it is not measured usage and receives no residual/classification refund.
...
A goal's single budget-classification checkpoint belongs to the admitted run that first performs classification, which may be a resumed continuation if the initial queued run was paused. Its hash, owner/goal/run binding, unique selection, allowance, and budget cycle remain mandatory. Classification usage stays on that actual run; cancelled predecessors remain uncharged, and renewal never reclassifies or rewrites the checkpoint.

New goals use budget policy 2. The utility model selects **small**, **standard**, or **extended**; at the default 384,000-token operator ceiling these allowances are **64,000 / 192,000 / 384,000** tokens. An unavailable classifier falls back to extended, within the operator ceiling. These are total run-cycle allowances, not visible-answer lengths or per-call output caps. Sizing must accommodate the next invocation's bounded exposure as well as already consumed tokens.

Explicit renewal grants exactly the previously selected allowance: the new lifetime ceiling is `consumedTokens + tokenAllowance`, with no unused-token rollover. Lifetime consumption and classifier accounting never reset. Existing goals retain their creation-time ceiling and selected allowance when configuration changes; renewal does not silently upgrade them. Selected policy-1 goals and historical renewal receipts remain intact; their UI continues to show the truthful lifetime budget rather than invent a cycle baseline. Migration 45 upgrades only pending policy-1 selections; migration 46 adds default-off consent and nullable grounding metadata. Both require the material-state writer drain, paired recovery point, and migration rehearsal below. Do not reverse them over retained new-format state.
...
The result is explicitly `kind: "recent-page-evidence"`. Every row carries current page identity, exact source revision and update time, an exact bounded opening prefix of source content, its full source character count, a truthful `contentTruncated` flag, and one page-level citation. It does not repeat derived knowledge projections, authority payloads, descriptions, OKF resources, full tables of contents, or section citations. The provider receives a smaller projection containing only the fields needed for the recap and citation. A delivered new-format row is current-run page evidence; historical metadata-only recent results are never promoted.

Catch Up uses that evidence action once with a limit of ten. It cites every returned page and does not fan out into ten `pages.get` calls merely to make the recent rows citable. A host-controlled notice discloses bounded opening-excerpt scope whenever any selected source was truncated. The recap must not claim coverage of an unread suffix or describe a revision diff: the action supplies current excerpts and update times, not historical comparisons. A successful bounded recap completes normally and is not an `AGENT_CONTEXT_TOO_LARGE` outcome.

Generic whole-result capacity handling remains unchanged. The engine admits a complete projected action result only when it fits the prospective provider context plus synthesis and correction reserves. An executed result that cannot fit is recorded as `status: "omitted"` with `reason: "tool_result_capacity"`; later calls prevented by that condition are `status: "not_executed"`. Every model-requested action still has exactly one `tool.started` event and one terminal `tool.completed`, `tool.notExecuted`, or `tool.failed` event. Omitted and not-executed outcomes carry strict `contextExclusion` data and are not failures.

Only delivered source evidence enters the citation registry. Delivered `pages.get`, `pages.getVersion`, and `pages.getOkf` results retain their existing evidence semantics. Delivered new-format recent-page evidence registers each returned excerpt separately under its exact page/revision citation; search, discover, related-page, and old recent metadata remain candidate metadata requiring a direct read. Omitted, not-executed, failed, malformed, or metadata-era outputs cannot be cited.

Factual citation validation is clause-local and keeps the existing 0.6 lexical threshold: support must come from one intact source sentence, list item, table row, or presentation unit within the exact delivered citation scope. Section identity follows the citation label's complete heading ancestry and fails closed when repeated headings remain ambiguous; it never guesses from citation position or a generated anchor slug. Parent sections retain descendant content, including nested details whose exact entering heading stack is restored on close, while child citations retain their identifying heading ancestry without borrowing parent or sibling body facts. Named and short identifiers are compared source-locally without capitalization bypasses. Numeric and mixed-digit claims bind to faithful source assignment segments while allowing an intact listed assignment to be summarized alone; polarity and body temporal/restriction qualifiers retain their local attachment. A heading qualifier such as `Current` may identify its child fact, but it does not erase or replace a qualifier required by the source body. Source units remain validation evidence regardless of character length.

Explicit structural lists may combine exact delivered headings, summary containers, link labels, and list members within the cited scope and the uniquely identified source container. This proves membership, not attached dates, discounts, availability, or other body facts. Table rows remain intact factual units; their cells cannot be recombined as independent members. Embedded link labels do not acquire their containing heading's identity. Rendering attributes and decorative icon suffixes are presentation rather than evidence words, while ordinary braces, identifiers, qualifiers, and literal fenced content remain intact. Shared fence rules prevent code from fabricating headings, and coherent claim/source boundaries keep abbreviated dates attached to their subjects.

Initial and correction prompts use Markdown heading blocks for organization and separately cited source-local assertions for substantive points. A page-level marker widens eligible source scope, not permission to pool unrelated facts or invent broad descriptions. Wording repairs reuse delivered evidence while preserving requested topic coverage, supported claims, and genuine gap disclosures.

When an otherwise substantive summary fails this gate, an existing evidence-correction turn receives at most 1,200 characters of bounded untrusted JSON containing complete rejected clauses and deterministic, complete exact source units from those already-delivered scopes. Unsupported citation-bound factual clauses cannot be published as advisory warnings; original uncited recommendations and organizing prose remain allowed. The packet reserves room for distinct cited scopes, favors locally relevant intact units, and only then adds other units. A unit that does not fit is omitted whole rather than truncated, without removing it from citation validation. The model regenerates the answer and the ordinary citation gate reassesses it; the host does not write the final summary, add an attempt, consult a truth oracle, broaden authorization, or bypass revision checks.

The root answer correction also receives a bounded, distinct sample of host-generated validation issues from both the beginning and end of the rejected draft. This tells the model whether the failure was an unsupported citation, altered link or number, uncited prose, or insufficient page-summary coverage without treating source excerpts as instructions. The evidence gate still independently checks the regenerated answer; tool admission, page authorization, and quota settlement are unchanged.
...
1. Identify the current application image, resolved configuration, persistent mounts, service identity, network, ports, enabled capabilities, and any unique files in the container writable layer. Keep this information private where it includes configuration or secret paths.
2. Run changed-path tests, applicable typechecks, and a successful application build. Tag the current image as a rollback reference when practical.
3. If the change has no destructive or irreversible migration and does not materially change persistent state, storage layout, key handling, or writer compatibility, use normal graceful application shutdown. Do not require a full writer drain, empty durable ledgers, PostgreSQL dump, or complete recovery snapshot.
4. Before replacement, preserve any unique writable-layer data that the new container would otherwise lose. Change only the application image and recreate only the `wiki` service with dependencies excluded. Preserve PostgreSQL, volumes, data, configuration, secrets, keyrings, service names, network, restart policy, and ports.
5. Start the application, verify health and ordinary data continuity, then exercise one focused runtime or UI flow covering the changed behavior. Record the tested commit/image and observed result; do not create release attestation or provenance artifacts.

For rollback, preserve current volumes and database state and use the prior image only when it is compatible with the resulting schema and persisted formats. If compatibility is uncertain, disable the smallest failing capability and fix forward. Never clear positive settlement intents, discard authoritative ledgers, prune volumes, run destructive down migrations, or restore an older database merely to make deployment pass. A state-rewinding restore requires explicit operator authorization acknowledging intervening writes.
...
Repository-disabled defaults are not a statement about the maintained installation: inspect the resolved configuration privately. Preserve the maintained settings. Establish a broad admission gate only when this comprehensive procedure applies; a normal Sprint restart may use the smallest narrowly necessary pause.
...
6. After health and continuity checks, exercise the acceptance surfaces required by the named Enterprise Release or material-state change. The listed three starters, custom/follow-up isolation, discovery, Catch Up capacity, activity/reconnect, accounting, and safe failure logs are a comprehensive Agent release set, not a routine Sprint checklist. A healthy container or HTTP health response alone is not acceptance, and no live success is implied by this runbook.
...
The historical release-candidate procedure recorded two migration rollback paths. They remain historical evidence only; select the Development Sprint rollback or the Enterprise/material-state procedure above from the actual risk. Do not enter these destructive historical paths for a routine code-only deployment:
...
Before the first goal run executes, the utility classifier chooses the **standard** or **extended** preset. **Standard** grants `floor(configured maxTokens / 2)` tokens; **extended** grants the full configured `maxTokens`. If the optional classifier cannot be admitted before provider dispatch, the bounded fallback selects **extended** without fabricating usage. Invalid classifier text with a valid provider receipt also falls back to **extended** while charging the full receipt; uncertain post-dispatch accounting remains a hard failure.

Tier selection and a hash-bound cumulative `usage.updated` classifier checkpoint commit atomically. Recovery requires exactly one valid checkpoint for a selected modern goal, reconstructs its usage once, and never counts it as a root or specialist model turn. The selected tier and allowance are immutable within a budget cycle; model output cannot change them.

Only a durable token fence can expose renewal. This includes measured exhaustion and a pre-dispatch refusal when the next bounded provider exposure cannot fit the remaining allowance; tool-call, continuation, duration, authority, and child-only limits remain nonrenewable. After explicit confirmation, a renewal adds one persisted tier allowance using `max(previous maxTokens, reconciled consumed tokens) + tokenAllowance`, preserving unused capacity without hiding an overrun, and advances `budgetCycle`. Renewal is never automatic.

Renewal admission requires the current goal version, a run ID, a client request ID, and confirmation. The allowance update, continuation admission, and hash-bound `run.resumed` operation receipt commit in one transaction. The receipt binds the original goal ID, expected version, run ID, client request ID, confirmation, and resulting cycle/ceiling. Repeating that exact request replays the existing continuation without a second grant; a changed expected version, ordinary-resume identifier, stale version, or reused identifier with different input is rejected.
...
Terminal Gemini usage accepts bounded `model_invocation_token_counts` and `non_grounding_model_invocation_token_counts` metadata. These do not replace authoritative input, output, or total token counters and are not summed into them. Unknown usage fields and oversized invocation/modality arrays still fail closed.
...
### Deployed Catch Up evidence and historical regression

Development Sprint image `local/tsepistle:catchup-d95de38e` was deployed to the maintained local tailnet with only the `wiki` service recreated. Live run `62c3463b-e929-4dc3-96c0-a7bcd0779655` completed with one `pages.listRecent` activity, ten page/revision citations, one brief entry for every returned page, and the host-controlled bounded-excerpt disclosure. It recorded no redundant `pages.get` fanout, omitted result, not-executed call, or failed activity.

The earlier deployed **Catch Up** regression recorded **11 durable activities: 3 delivered, 1 omitted, 7 not executed, and 0 failed**. Its final `evidence.provenance` cited only delivered evidence; the assistant message was persisted with status `complete`, while the run was terminalized as `partial` and its completion assessment was `outcome: "blocked"` with `AGENT_CONTEXT_TOO_LARGE` and `retryable: false`. The seven activities previously described as failures were capacity-skipped calls, and the one omitted activity was an executed result withheld from provider context; neither was a genuine provider or Wiki-action failure. The cause was redundant orchestration: a metadata-only recent list repeated derived projections, then ten requested `pages.get` calls repeated page evidence under a conservative serialized-request capacity bound.

The earlier third-turn provider-request/continuation report remains historical context, but it is a separate incident from that capacity regression and must not be used to label it a provider failure. For current operations, capacity is established by explicit `tool.completed`/`tool.notExecuted` dispositions and completion assessment; a genuine provider or action failure requires its own `tool.failed` event and allowlisted failure telemetry.
...
When intentionally using the registry publication lane or an activated Enterprise Release, treat `ghcr.io/philosophimoonbeam/wiki-canary-promotion:canary-set` as the only commit point for the application/browser-worker set. These registry and multiarchitecture instructions do not govern a locally built Development Sprint image. The ordinary `:canary` image tags are non-authoritative conveniences and can temporarily name different runs while publication is in progress. Resolve the set once with the checked-in verifier; it validates the record, its exact main revision, all four architecture descriptors, and the matching immutable record keyed by that revision:
...
### Enterprise Release security review and attestation gate

Only an explicitly activated official beta, production release, or compliance milestone is gated by the executable security check (`bun server/scripts/check-threat-model.ts --release`). Release qualification is strictly separated from Development Sprint and is owned by the repository maintainer or a delegated agent. Routine commits and maintained local-tailnet deployments do not create source-review records, attestation successors, release evidence, tags, or provenance artifacts.
...
- **Rollback:** select the Sprint or Enterprise/material-state rollback path from the actual compatibility and persistence risk. Disable the smallest failing capability when safe; use broad drains, backups, or historical compatibility paths only when their stated preconditions apply.
