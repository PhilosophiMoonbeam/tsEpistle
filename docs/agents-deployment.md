...
When an otherwise substantive summary fails this gate, an existing evidence-correction turn receives at most 1,200 characters of bounded untrusted JSON containing complete rejected clauses and deterministic, complete exact source units from those already-delivered scopes. The bound prioritizes one relevant unit across failed clauses before additional units. A unit that does not fit is omitted whole rather than truncated, without removing it from citation validation. The model regenerates the answer and the ordinary citation gate reassesses it; the host does not write the final summary, add an attempt, consult a truth oracle, broaden authorization, or bypass revision checks.
