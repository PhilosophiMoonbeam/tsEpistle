...
| DATA-1 | Unknown, partial, newer, or locked database is mutated | Non-mutating migration preflight before legacy bridge/current migrations; ordered ledger and sentinel checks | `server/core/db.ts`; `server/db/migration-preflight.ts`; `server/test/db/migration-preflight.test.ts` | Retained upgrade/restore fixtures for every supported database are required. |
