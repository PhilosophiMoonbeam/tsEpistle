...
11. explicit retirement deploys the `/sw.js` tombstone, suppresses registration in ordinary and neutral bootstraps, proves unregister and exact owned-cache cleanup, replaces only currently safe controlled clients, preserves unsafe clients' memory, and leaves all IndexedDB snapshots/search/drafts/receipts and unrelated caches intact.
...
Rollback MUST account for browser-owned service-worker state. Retain a separately deployable JavaScript tombstone artifact at the exact `/sw.js` URL and root scope, served with `Content-Type: application/javascript` and no-cache update headers. Retirement suppresses registration in both ordinary and neutral documents. The tombstone is the explicit `skipWaiting` exception, deletes only recognized owned worker caches, and attempts unregister independently of cache-cleanup or navigation failure. Only relevant, currently safe controlled windows may replace themselves with network documents. Unsafe, frozen, missing, or unacknowledged windows are not forcibly navigated; existing documents retain their memory for explicit resolution. Uncontrolled windows are not redirected. Retirement does not delete or migrate IndexedDB. A prior image may be selected only after its server-writer, physical-schema, and key-domain compatibility is proved; it must ignore unknown newer IndexedDB schemas rather than delete drafts.

Browser proof exercises safe and unsafe controlled windows, unregister despite cleanup failure, no re-registration loop, preservation of every IndexedDB store and unrelated cache/window, and restored feature operation. A received 404, removed route, blanket cache deletion, or application-only unregister is not retirement proof.
