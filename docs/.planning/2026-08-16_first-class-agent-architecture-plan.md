...
- **Provider protocol UX container replacement in progress (2026-08-17):** rebuild the committed provider-protocol labeling/defaults revision, replace `wiki-tailnet` while preserving its PostgreSQL/content/network/secret/restart contract and loopback publication `127.0.0.1:3013`, and retain `https://agents8c48g.tail41a24a.ts.net:10443/` without changing the Tailscale Serve route. Record the resulting source revision, image digest, backup checksum, rollback container, health, and browser evidence here after cutover.
