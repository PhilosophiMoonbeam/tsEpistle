...
- **Status:** implemented product baseline with the current maintained-canary recovery reconciliation applied; ordinary-Wiki inline architecture remains authoritative, and release evidence is revision-bound.
- **Date:** 2026-08-17
- **tsFranki revision assessed:** `05ee166091c0858f5df231a7f82cec960e2befe16` (`docs: clarify MCP approval boundary`)
- **Historical Ax assessment:** `@ax-llm/ax` `23.0.15`, source revision `3ff5ff4689f01afc1d8498a64f698bc5e5a3cf6a`.
...
- **Evidence status:** Evidence attributed to earlier isolated-origin revisions remains historical and does not satisfy the corrected source-review/release gates. The Gate J/general-availability checklist below is a future published-release target; the maintained-canary recovery has a separate source-freeze, clean-artifact, controlled-cutover, and live-acceptance sequence. This plan makes no final-image or live-provider success claim.
- **Historical installed-deployment baseline:** The ordinary Wiki origin exposed Search and Ask inline. In that baseline, provider inference, skills, browser tools, proposals, writes, and MCP were controlled by independent disabled-by-default settings; repository defaults are not a statement about the maintained canary's installed configuration.
- **Current maintained-canary recovery:** The corrected implementation is intended for a controlled replacement of the maintained canary. It preserves the ordinary origin, PostgreSQL state, and policy/action authorities while admitting request-scoped providers, exact-origin bounded continuation, atomic provider/skill admission, bounded resources, paired activity, flat next-turn discovery, and collecting-to-synthesis.
- **Historical deployment entries:** The dated container-replacement entries that follow are retained as prior evidence and are not current maintained-canary or general-availability claims.
- **Installed container replacement completed (2026-08-17):** promoted image `wiki-tailnet:9f5a577b` / `sha256:29ca6d60a119ac63b528353980d7613497e043bee6d7c535a6d8bbf82953451b` from source revision `9f5a577ba2665d254b4870d17685aad380f77958`. The deployment retained PostgreSQL 17.11, `wiki-postgres-content`, `wiki-pg-migration-net`, loopback publication `127.0.0.1:3013`, restart policy `unless-stopped`, and tailnet URL `https://agents8c48g.tail41a24a.ts.net:10443/`. Pre-cutover database backup `wiki-pre-agent-9f5a577b.dump` has SHA-256 `406fe2da67221a023ab2b598ee07c995ac8125d8ea637f122f2b4d48f18abfce`; stopped container `wiki-tailnet-pre-9f5a577b-20260817T161723Z` is the immediate application rollback target. Migration `2.5.140.js` applied and removed the obsolete launch-handoff table. The promoted container reached Docker `healthy` with zero restarts, returned `{"ok":true}` on loopback and tailnet `/healthz`, rendered `Page Home | Wiki.js` with the inline Ask control through tailnet HTTPS, and repeated those startup/health checks after an explicit container restart.
...
This plan defines agents as a tsEpistle product capability, not as a chat widget attached to a privileged backend. The same policy-aware action kernel will serve the in-product Ax agent and authenticated external MCP clients. tsEpistle remains authoritative for identity, page visibility, permissions, approvals, persistence, audit, quotas, and transport. Ax owns model orchestration only. The MCP SDK owns protocol conformance only. CopilotKit is a UX and interaction-state reference only and is not a dependency.
...
11. **Borrow coding-harness safety contracts, not their runtimes.** Codex contributes preflight/atomic patch semantics and durable response-item history; Oh My Pi contributes snapshot-tagged line anchors, all-or-nothing patching, progressive skill injection, and append-only custom session state. tsEpistle implements those contracts over pages and PostgreSQL.
...
- External agents can use permission-scoped tsEpistle actions and list/read approved skill snapshots through MCP with the existing API-key model. Their host owns its conversation transcript; tsEpistle owns its skill-use, proposal, approval, execution, and audit records.
...
- Letting the tsEpistle agent connect to arbitrary remote MCP servers.
...
- An MCP “ask the tsEpistle agent” tool. MCP exposes Wiki actions and approved skills; it does not nest another paid agent loop.
...
- The assessed baseline was PostgreSQL-only, used Node.js 24 in its build/deployment flows, and used additive Knex migrations with migration preflight and upgrade smoke coverage. The Node.js 24 wording is historical; current toolchain/runtime authority is the pinned Bun workflow in `package.json`, `bun.lock`, and `.github/workflows/build.yml`.
...
- `/a`, `/p`, and ordinary page routes remain ordinary Wiki shells. Agent REST is an authenticated internal API under `/_api/agents`; the inline user surface lives inside the existing search overlay, administration lives under `/admin/agents`, and MCP uses the exact `/mcp` endpoint on the same configured Wiki origin.
...
- Do not enable playbooks, memories across users, optimization, arbitrary skills, recursive agents, or discovery over unapproved tools in the initial release. The maintained design uses only flat discovery over the already admitted catalog and one next-turn enable control.
- Do not treat an Ax runtime snapshot as a stable product-session format. Wiki messages and events plus bounded provider-continuation envelopes are the durable contract. Continuation storage is dialect-specific rather than uniformly encrypted: provider-supplied encrypted payloads remain opaque where that dialect provides them; Gemini Interactions uses bounded canonical JSON replay state for validated `model_output`, `function_call`, and optional `thought` signature/summary steps, and its `encrypted: true` marker is not application encryption; OpenAI Responses may retain a bounded validated `rs_...` reasoning-item ID alongside its encrypted payload. The persisted origin-bound state carries a SHA-256 integrity check, not confidentiality, so a database operator with direct access can inspect locally encoded bytes. Arbitrary raw response payloads and plaintext hidden reasoning are not persisted as normal conversation/event content, and continuation bytes are excluded from normal logs/UI. A later request accepts one only when the originating owner, session, immutable provider-profile version (and therefore its origin), transport, model, and capability revision match, and the state passes schema, dialect, size, and integrity checks. A mismatch or unsupported state is omitted and rebuilt from canonical Wiki history; corruption fails closed. This supports bounded cross-request continuation without delegating transcript ownership or authorization to provider state, while exact Ax runtime-snapshot continuation remains unsupported.
...
- Provider profiles store transport kind, approved HTTPS base URL, an opaque encrypted-credential reference, adapter-defined authentication mode, model allowlist, context/output limits, feature flags, timeout, retry policy, and pricing revision. The ordinary admin form accepts an API key only on immutable version creation; the server encrypts it transactionally and never returns it. Never accept arbitrary per-user base URLs, headers, model IDs, or provider-native tools.
...
- Do not use provider-side conversation storage as the source of truth. Send bounded Wiki history each run and request non-storage where the transport supports it. Remote conversation, thread, and response IDs never become resume authority; a bounded validated reasoning-item ID may assist replay only when nested in a matching origin-bound envelope, and it is not a resume token.
- Normalize output deltas, tool calls, finish reasons, refusals, usage, retry metadata, and provider errors into one Wiki contract. Persist only bounded provider-native continuation envelopes for supported dialects on the originating assistant message: depending on the dialect, these contain provider-encrypted opaque payloads or structured canonical JSON replay state (including Gemini `model_output`/`function_call`/optional `thought` steps), and OpenAI Responses may retain a validated bounded `rs_...` item ID. A later request accepts one only when the exact originating owner/session/provider-profile-version/origin/transport/model/capability identity matches and schema, dialect, size, and integrity checks pass; otherwise it omits the state and rebuilds from canonical history, failing closed on corruption. The canonical Wiki transcript, action records, authorization, and provider admission remain authoritative. Arbitrary raw provider payloads and plaintext hidden reasoning are not persisted as normal conversation/event content, and continuation bytes are excluded from normal logs/UI.
...
- Secrets remain server-side. Health checks verify keyring/configuration readiness and explicit administrator-triggered model probes; ordinary `/healthz` never decrypts a provider credential, sends Wiki content, or makes a provider call.
...
- Metadata is the discovery layer. Full instructions load only after explicit session selection or an authorized `skills.read` of a run-pinned version. Referenced files load individually on demand. `scripts/` may be stored/exported but are never executed by tsEpistle in the initial release.
...
- External MCP clients can list metadata and read exact raw files through tools and `wiki://skills/{name}/{version}/{+path}` resources. The reserved expansion permits nested bundled paths; the server decodes once and applies the same strict relative-path validator as tool reads. Their Codex/Oh My Pi/other host persists the returned tool/resource content in its own transcript; tsEpistle deliberately does not import or impersonate that external conversation.
...
- Use the SDK’s Host and Origin validators. Independently bind the API-key token's versioned MCP resource claim to the canonical MCP URL derived from the configured Wiki origin before constructing `AuthInfo`; those middleware do not validate the resource claim.
...
tsEpistle will implement the same useful concepts in native Pinia/Vue:
...
- Zod is not currently a direct tsEpistle dependency. Add and exact-pin a Zod 4 release during Phase A; include it in lockfile, license, SBOM, and upgrade review.
...
- for tsEpistle: one-page scope, no path syntax, bounded operation/line/byte growth, uniform line-ending restoration, result Markdown validation, and canonical result hashing.
...
`agentProviderProfileVersions` is immutable and stores profile/version, transport/model/base URL/auth/opaque managed-secret reference, canonical adapter config, measured capabilities/revision, policies/pricing, conformance, actor, and time. `agentProviderSecrets` stores AES-256-GCM ciphertext, a unique nonce, authentication tag, and encryption key ID; no read projection returns those fields or decrypted credentials. Runs retain the profile-version FK. Session/profile projections return a versioned shared-key MAC over session profile identity or null, resolved version, profile policy version, default generation, and execution mode. Admission re-resolves under lock, verifies the MAC and live grants/status, and rejects stale resolution before egress.
...
Provider-native thread, conversation, and response IDs, remote conversation retention, Ax memory, and prior in-process program instances are never authoritative. A bounded reasoning-item ID inside a validated origin-bound envelope is replay metadata only, not resume authority. The canonical Wiki transcript is always the source for bounded context. A supported provider-native continuation envelope may be attached to its matching prior Wiki assistant turn on a later request only when exact owner, session, immutable provider-profile version/origin, transport, model, capability revision, dialect, and integrity checks match; an unsupported or mismatched state is omitted and the turn is rebuilt from Wiki history, while corruption fails closed. OpenAI Responses continues to avoid `previous_response_id` as a product authority; Chat Completions, OpenResponses, and Anthropic receive their native translations of the canonical input plus any eligible continuation. Tool call IDs are transport-local aliases mapped to Wiki action-call IDs and are not trusted after the attempt boundary.

`AgentEngine.run(input)` creates one Ax agent/program, installs only catalog functions allowed for that run, and lets Ax iterate model -> function calls -> validated results -> model until a typed final response, limit, cancellation, approval pause/resume, or terminal error. The action wrapper persists the call before execution and returns a bounded result envelope. Calls in each model batch are serialized in model order; a write proposal never executes during model generation, and synthesis runs without actions.

#### Flat discovery and collection-to-synthesis

Tool exposure is a flat view of the admitted action catalog, not a second registry. During `collecting` turns with an admitted action session, core actions are visible. During `synthesizing`, no actions or `wiki_enable_tools` control is exposed. When admission produces nonempty non-core groups, the engine exposes one category index containing only admitted `explore`, `history`, `canonical`, `authoring`, and `browser` categories. The sole `wiki_enable_tools` control is itself a read-only control activity: it requests one admitted category, records paired activity, and makes that category visible only on the next model turn. It is not an action-kernel/MCP authority, permission grant, nested discovery call, or same-turn schema expansion.

Before completing an enable, the engine prospectively builds the expanded next-turn catalog and system prompt and checks provider context/result capacity. If that expansion cannot fit, the enable is not committed and the control receives an explicit omitted/not-executed result.

While `collecting`, admitted calls execute serially in model order. Each source result is bounded and whole; the candidate, remaining not-executed envelopes, and synthesis reserve are preflighted before delivery. Capacity triggers explicit omitted results, paired terminal activity, a coverage notice when evidence is omitted, and an irreversible `synthesizing` phase with no action or control tools. The engine then runs one monotonic bounded tool-free synthesis phase from the available evidence; an evidence-rejected draft may receive additional bounded tool-free correction turns within the existing turn/token limits before acceptance or terminal error. When capacity omitted evidence, the completion is partial/blocked (including `AGENT_CONTEXT_TOO_LARGE` when applicable); without a capacity omission, the ordinary complete assessment remains possible. It never resumes collection, repeats collection for the same goal automatically, or claims complete coverage when evidence was omitted. This is one engine state machine, not a competing orchestrator.

Prompt assembly uses one typed, provenance-delimited `AgentRunInput` consumed by `AxAgent`; canonical PostgreSQL history is a field of that input, not an attempted injection into Ax memory. All user/page/skill/browser strings are encoded as data values rather than interpolated into policy markup. Priority is product policy, action contract, selected skill instructions, canonical conversation, tool evidence, current request. Transport adapters serialize the bounded current Ax call plus any validated exact-origin provider continuation attached to matching canonical assistant turns. They may map prior Wiki turns into the provider's native request dialect as pure serialization, but they never create a second transcript or accept opaque state from a different origin. Golden tests freeze field ordering and delimiters.
...
| --- | --- | --- | --- |
| `openai-responses` | Wiki Responses `AxAIService` adapter using public Ax contracts | `POST /v1/responses` | preferred OpenAI path; `store:false` requested where supported, but provider processing, logging, retention, and deletion remain outside Wiki control; exact-origin bounded continuation; action/tool continuation remains attempt-local |
...
`AgentEngine` constructs a fresh service per run/attempt so mutable model/parser/continuation state cannot leak across users. A validated bounded provider continuation may be rehydrated only from the matching canonical assistant turn and exact origin identity. It translates the live Wiki catalog to Ax functions with Standard Schema inputs, supplies abort/deadline, and invokes Ax with retries disabled; the coordinator alone schedules durable attempts. Streaming agent mode uses `streamingForward()`; buffered generation-only mode uses `forward()` with no functions. Both feed the same durable reducer. Ax can request a function only in agent mode; only the Wiki wrapper executes it.

#### Transport contracts

- **OpenAI Responses:** send bounded local input with `store=false`; do not use `previous_response_id` as a product authority. When conformance supports reasoning/function loops, attach only the exact-origin, hash-validated, bounded dialect envelope from a matching prior Wiki assistant turn and retain/replay its validated ordered output items—including the provider's opaque encrypted reasoning payload—within the active call before matching function results. For OpenAI Responses, the persisted envelope may retain a bounded validated `rs_...` reasoning-item identifier alongside that encrypted payload; Gemini Interactions instead persists bounded canonical JSON replay steps (`model_output`, `function_call` arguments, and optional `thought` signature/summary), where `encrypted: true` is a schema marker rather than application encryption. Persist only the bounded validated envelope with the assistant message for later turns. Arbitrary raw response payloads and plaintext hidden reasoning are not persisted as ordinary conversation/event content, and continuation bytes are excluded from normal logs/UI. Remote conversation/thread/response IDs never become resume authority. If state is mismatched or unsupported, omit it and rebuild from canonical Wiki history; corruption fails closed.
- **OpenResponses:** use the same bounded canonical-history and exact-origin continuation rule and Wiki function schemas, but its own generated request/event validators. Reject unknown required event shapes rather than silently treating a malformed stream as a final answer. Record the exact OpenResponses schema revision in the profile capability revision.
- **OpenAI Chat Completions:** serialize the bounded canonical Wiki input and any eligible exact-origin continuation through the native request dialect and Wiki functions/tools; parse `tool_calls`, incremental argument fragments, finish reasons, usage, and refusal/content where supplied. Attempt-local tool results use native call/result messages. Do not emulate Responses-only remote continuation, item IDs, or built-in tools.
...
- The admin browser sends an API key only in the CSRF-protected profile creation/version request over the ordinary HTTPS origin. The server immediately encrypts it with AES-256-GCM under a deployment-managed versioned keyring, authenticated to a random managed-secret ID, and stores only ciphertext.
- The admin page never receives an API key, secret reference, ciphertext, nonce, tag, or decryption key. It displays configured/unconfigured state only; editing creates a new immutable credential and profile version.
- Provider-credential encryption keys and profile-resolution MAC keys use separate dedicated keyrings with current key IDs and rotation overlap. Resolution tokens contain only identities/revisions/mode, never provider credentials. Removing a provider encryption key that remains referenced fails credential resolution closed.
...
- Base URLs require HTTPS and initial policy validation. Provider traffic uses the injected guarded fetch for the exact configured public HTTPS origin and allowlisted base path; it rejects URL credentials and all private/loopback/link-local/CGNAT/multicast/reserved addresses, pins public DNS/address resolution, omits ambient credentials, sends only the profile's configured provider authentication and allowlisted additional headers, rejects every redirect rather than following it, and never hands unchecked destinations to global fetch.
...
Serve `app.all('/mcp', ...)` on the configured Wiki origin with its own bounded parser before ordinary user authentication and page routing. The canonical resource is always `${wiki.config.host}/mcp`; there is no second MCP origin or independently configurable resource URL. The route is absent while MCP is disabled and reserved for MCP while enabled, so a Wiki page at `/mcp` is available only when MCP is disabled. Host/Origin/certificate/canonical-resource preflight must pass before the feature route activates.
...
5. Run the official Host and Origin validators as separate checks. Require a versioned `mcpResource` claim on the API-key JWT, normalize it, and compare it to the canonical `/mcp` URL derived from the configured Wiki origin. Existing tokens without the claim remain valid for existing APIs but are rejected at MCP until regenerated.
...
MCP is disabled by default and exposed only through exact-path ingress, normally private during rollout. When disabled, `/mcp` falls through to ordinary Wiki routing. When enabled, the route rejects browser sessions and requires a resource-bound API key. Rate limits apply by API key, IP, and tool risk.
...
| token/provider secret leakage | authenticated encryption at rest with deployment keyring, write-only admin input/read-only readiness projection, request-body error output disabled, strict allowlisted error mapper, redacted logs/events, no token in `AuthInfo.extra`, bounded error text | database/profile/API projections contain no plaintext; nonce/tamper/missing-key tests fail closed; failure paths prove request/response bodies, credential URLs, tokens, and content never reach logs/events/traces |
| provider remote retention/privacy surprise | local transcript is authoritative; `store:false` is requested where supported but is only a request preference, not proof of provider-side non-processing, non-logging, non-retention, or deletion; the provider receives bounded Wiki prompts/source/tool data and any eligible continuation under configured authentication; provider conversation/thread/response IDs never authorize resumption; profile UI states destination/storage; only validated exact-origin local envelopes may assist later turns | request fixture asserts storage fields and session replay works after remote/provider state loss |
| SSRF via provider/MCP config | provider traffic uses HTTPS exact configured origin/base-path validation, public DNS/address pinning, and rejection of every redirect; MCP uses a host allowlist | DNS rebinding, redirect, userinfo, private/link-local, and mixed-address tests |
...
| route shadowing | internal `/_api/agents` handles only its exact prefix; `/mcp` falls through while disabled and is intentionally reserved while MCP is enabled; `_agent` remains an ordinary page | disabled `/mcp` page, enabled MCP reservation, `_agent`, and unrelated routes before/after enablement |
...
### Bounded configuration and focused checks
...
The Beads, integration gates, rollout sequence, and release matrix below are retained from the original design roadmap. They describe the future full-feature/general-availability target, not completion of the current maintained-canary recovery. Any signed-review wording in the retained J3/Gate J material records the historical process; current release eligibility is maintainer-owned schema-2 policy under the authorities at the top of this document. Current recovery operations remain in `docs/agents-deployment.md`, without a second command sequence here.

## Implementation roadmap

### Agent hierarchy and ownership

Implementation uses one lead integrator and bounded specialists. The hierarchy is organizational; no runtime “agent swarm” is added to tsEpistle.
...
| --- | --- | --- | --- | --- |
| A1 Shared contract/dependency freeze | contract/data | none | exact dependencies; shared contracts; ordinary authenticated internal agent API, same-origin exact-path MCP rules, and license/SBOM inputs | typecheck; package/lock/license diff; trust-boundary review |
...
| A3 Inert inline shell/settings | contract/data + client | A1,A2 | flags false, inline Ask surface and ordinary `/admin/agents`; no separate agent entry, login, host, or launch route | startup/page/admin smoke; disabled `/mcp` fallthrough, `_agent`, and zero-egress checks |
...
| E2 OpenAI transports | inference | E1 | Responses service with bounded exact-origin dialect-specific reasoning continuation plus Ax Chat factory; storage-off typed mapping and one-attempt parser | reasoning/call/result, include field, fragments/refusal/usage/abort/429 |
...
| I1 MCP transport/auth | MCP | A2,C1 | same-origin `/mcp`, parser, official v2 adapter, resource-bound auth | disabled route fallthrough; enabled reservation; Host/Origin/resource/legacy tests |
...
| J2 Deployment/documentation | lead/release | F5,G1,J1 | ordinary Wiki with exact-path MCP ingress, app/browser/maintenance container procedures, N-1 compatibility, and runbooks | install/auth/upgrade/rollback/drain and retention during N-1 |
...
#### Gate J — general availability (historical target; not current canary status)

The original GA target required every A1–I3 Bead and J1–J3, including F5. Full quality, build, upgrade, database, multi-instance, dedicated-host ingress, browser/accessibility/performance, multi-arch images, package/license/SBOM/checksum/manifest/provenance, retention-maintenance, and rollback evidence had to identify one commit. Test fakes remained test-only. This is a future published-release criterion, not evidence that the maintained-canary recovery is complete.

## Rollout (historical feature sequence; not the current recovery procedure)
...
11. **General availability (future only).** Remove rollout-only canary allowlists after evidence, not kill switches, conformance state, version pins, or audit controls.
...
- typed input ordering without duplicated trigger; Responses bounded dialect-specific reasoning continuation (including validated item IDs and opaque/structured state); dialect/error/usage transforms;
...
- ordinary user session versus guest/API-key, hostile Origin/Fetch-Metadata, CSRF, privileged `extra.js` trust boundary, sanitized agent output, disabled `/mcp` fallthrough, enabled MCP reservation, and `_agent` page continuity;
...
- actual startup with all flags disabled, exact-prefix fallthrough, `/mcp` page continuity, and ordinary route/health behavior; enabled startup with same-origin MCP Host/Origin/resource enforcement;
- fake engine through ordinary-origin REST/SSE with cancel/reconnect/quota/restart;
- provider corpora, getFeatures/descriptor equality, bounded dialect-specific Responses/Gemini continuation, distinct OpenResponses request/event validation, legacy rejection, and one controlled real read smoke;
...
### Release (historical GA evidence target)
...
- container install/upgrade/rollback with ordinary Wiki and exact-path MCP ingress, SSE, maintenance, and optional browser worker;
- historical AMD64/ARM64 app/browser/maintenance OCI plus ordinary Linux/Windows bundles with unsupported capability absent; the Windows wording is retained roadmap context. Current supported binary artifacts are Linux-only, as defined by `.github/workflows/build.yml` and `README.md`.
...
- record user-visible behavior and migration/settings changes in the normal release notes when publishing an official release; the current maintained-canary recovery does not publish one.
...
- ordinary internal agent controller plus same-origin exact-path MCP composition;
...
## Final acceptance criteria (historical full-feature/GA target)

The following full-feature criteria remain the architecture's end state; they are retained from the original plan and are not a status claim about the current maintained-canary recovery. Current recovery acceptance and live evidence are governed by the current source-review, build, deployment, and operator authorities stated at the top.
...
5. Ax orchestrates only through registered policy-aware callbacks; transport adapters are explicit, conformed, version-pinned, use attempt-local mutable state plus bounded exact-origin persisted provider continuation (dialect-specific opaque encrypted payloads or structured Gemini state, with bounded reasoning IDs where required), and never expose hidden reasoning as ordinary stored/streamed content.
...
13. MCP uses official v2 packages at `${wiki.config.host}/mcp` and passes API-key/Host/Origin/resource, payload-bound idempotency, tools/resources, progress/cancel/modern/legacy tests.
