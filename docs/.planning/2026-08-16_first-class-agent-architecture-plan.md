...
- **Container replacement in progress (2026-08-17):** replace installed image revision `de07817f152eaa81b9f45c29e6227c8a79769331` with the current committed implementation while preserving PostgreSQL, `wiki-postgres-content`, the `wiki-pg-migration-net` network, loopback publication `127.0.0.1:3013`, and tailnet URL `https://agents8c48g.tail41a24a.ts.net:10443/`. Take a PostgreSQL backup before migration `2.5.140`, retain the stopped prior container as the immediate rollback target, and record the promoted revision and local/tailnet health evidence here after cutover.
