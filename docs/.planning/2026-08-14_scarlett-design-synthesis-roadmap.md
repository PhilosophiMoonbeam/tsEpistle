...
- render tabs, spoiler, and infobox as escaped semantic text rather than a second Markdown/HTML interpreter; browser behavior enhances the server fallback without changing its source;
- keep PDF and media references on same-origin asset paths so the existing asset authorization and range-serving boundary remains authoritative; never accept arbitrary player or iframe markup;
- render Mermaid lazily and locally, sanitize its returned SVG before insertion, and treat Kroki, PlantUML, YouTube, and OpenStreetMap as fixed typed providers with zero pre-consent requests, bounded URL construction, explicit sandbox/referrer policy, and durable source/link fallbacks.
...
- additive migration `2.5.138` adds disabled-by-default rows for tabs, spoiler, infobox, PDF, media, YouTube, Mermaid, Kroki, PlantUML, and OpenStreetMap without modifying earlier migrations; fresh, idempotent, downgrade, and additive-upgrade contracts pass;
- the shared discriminated schema and editor host cover the complete 13-extension catalog with strict property rejection, bounded values, canonical fences, same-origin asset paths, fixed provider identifiers, and compatibility diagnostics;
- static semantic renderers emit escaped accessible HTML; local media retains direct open/download fallbacks; Mermaid runs only on pages that contain it and sanitizes the generated SVG; remote providers retain inert source/link fallbacks until explicit reader consent;
- the focused extension suite passes 74 assertions across schema, migration, registry operations, sanitized rendering, API validation, and browser hydration; complete lint, client/server typechecks, 1,406-test Vitest suite, production build, and every bundle budget pass;
- a live SQLite browser smoke enabled all 13 registry entries, authored and rendered all ten new extension types, exercised tabs/spoiler controls, rendered Mermaid locally, confirmed zero active scripts and no horizontal overflow, then verified typed YouTube/Kroki/PlantUML/OpenStreetMap targets with external requests aborted;
- the release Playwright workflow now authors the complete extension catalog, proves no provider request occurs before consent, exercises every local and remote group, repeats the surface at 390×844 in dark/reduced-motion/forced-colors mode, and rejects serious or critical Axe findings; the focused Chromium workflow passes;
- all isolated browser databases, credentials, registry state, pages, and configuration files are temporary and removed after verification.
