...
import { parseMarkdownCodeFenceInfo, parseMarkdownCodeFences, renderMarkdownCodeFence } from './markdown-code-fence.ts'

describe('Markdown code fences', () => {
  it('parses, normalizes and merges supported fence metadata', () => {
    expect(
      parseMarkdownCodeFenceInfo('TypeScript title="Say \\"hello\\"" linesStart="30" linesHighlight="35, 31-33, 33-34, nope, 40-38"', value =>
        value.replaceAll('\\"', '"')
      )
    ).toEqual({
      language: 'TypeScript',
  ...
  it('classifies renderer-compatible fence indentation and marker lengths', () => {
    const source = '   ```` wiki-extension \r\n{"key":"indented"}\r\n  `````\r\n' + '~~~ text\n```wiki-extension\n{"key":"nested"}\n```\n~~~~\n'
    expect([...parseMarkdownCodeFences(source)]).toEqual([
      {
        marker: '`',
        markerLength: 4,
        info: ' wiki-extension ',
        content: '{"key":"indented"}\r\n',
        closed: true
      },
      {
        marker: '~',
    ...
    expect([...parseMarkdownCodeFences('    ```wiki-extension\n{"key":"too-deep"}\n    ```')]).toEqual([])
    expect([...parseMarkdownCodeFences('```wiki`extension\n{"key":"invalid-opener"}')]).toEqual([])
  })
  ...
  it('keeps a real two-line block numbered while leaving a plain single line compact', () => {
    expect(renderMarkdownCodeFence({ source: 'first\nsecond\n', info: 'text' })).toContain('class="prismjs language-text line-numbers"')
    expect(renderMarkdownCodeFence({ source: 'only one line\n', info: 'text' })).toContain('class="prismjs language-text"')
  })
