...
At the time of this historical handoff, the audit did not claim a repository-wide release attestation or arbitrary semantic recall. The historical 55f baseline and staged search audit were release-ineligible under the then-current review policy. This file preserves those facts only; the former policy is explicitly superseded by the current maintainer-owned `schemaVersion: 2` source-review policy, and neither historical record is current release authority.
...
1. **Run lint API release and build gates** (Closed / verified).
   ...
   - At this historical handoff, `ci:static` remained blocked solely by the manifest-driven attestation gate (`docs/security/review-attestations.json`), which had replaced the former monolithic Covered source mechanism in `threat-model.md`. The historical `55f30709cd4b8de0ec1fd498cac7174f1cacd084` baseline was represented by a release-ineligible structured record with blocking finding `SEC-EXT-001` (external reviewer unassigned under the superseded policy). The current search audit was represented by a `working-tree-audit` record using fingerprint `3adffcba7255a3f88ec490ceffefe6f826e9508a` (base `d3d5a63326c056bb847fdb2552be0a1cf1eb7020`), which was release-ineligible staged evidence rather than a release attestation. These were historical limitations, not current schema-2 release requirements.
...
5. **Finalize audit evidence and explicit release limitations** (Closed / completed).
   - Reconciled documentation across `docs/search-foundation-audit.md`, `docs/search-architecture.md`, `docs/agents-deployment.md`, `docs/search-agent-next-steps.md`, and `docs/security/threat-model.md` with authoritative evidence:
     ...
     - Explicit release limitations recorded at that time: the manifest-driven attestation gate blocker (historical 55f baseline represented by a release-ineligible structured record with blocking finding SEC-EXT-001 under the superseded external-review policy; search audit represented by a release-ineligible working-tree-audit record using fingerprint `3adffcba7255a3f88ec490ceffefe6f826e9508a`; release remained blocked until committed exact source received the then-required review record), actual `gpt-5.6-luna` held-out miss (`restore the ultraviolet verification code`), no universal semantic recall claim, operator-approved shared-content egress policy, separate background concurrency accounting, uncommitted worktree, and no deployment. The release-policy portion is historical and is superseded by the current maintainer-owned schema-2 gate.
...
- Under the manifest-driven attestation framework (`docs/security/review-attestations.json`), the former monolithic Covered source mechanism in `threat-model.md` had been replaced by structured review records. At this historical handoff, the **`55f30709cd4b8de0ec1fd498cac7174f1cacd084`** baseline was represented by a release-ineligible structured record with blocking finding `SEC-EXT-001` (external reviewer unassigned under the superseded policy). The current search audit was represented by a `working-tree-audit` record using fingerprint **`3adffcba7255a3f88ec490ceffefe6f826e9508a`** (base `d3d5a63326c056bb847fdb2552be0a1cf1eb7020`), which was release-ineligible staged evidence. These historical records remain evidence with their findings unchanged but are superseded by the current maintainer-owned schema-2 policy.
...
# Manifest-driven attestation gate (historical handoff command; the former external-review blocker and working-tree state described above are superseded by the current schema-2 maintainer policy):
...
5. Preserved the full detailed assessment, executed plan, OpenWiki comparison, receipt-fenced graph repair proof (distinguishing session-observed exact revision-1 recovery from retained `legacyRenderAndLinksRecovered: true` and empty `receipts: []`), benchmark figures, fingerprint `3adffcba7255a3f88ec490ceffefe6f826e9508a` (recorded as a release-ineligible working-tree-audit record, not a release attestation), actual `gpt-5.6-luna` held-out miss, and the historical manifest-driven attestation gate blocker (55f baseline release-ineligible with SEC-EXT-001 blocking under the superseded policy), plus uncommitted/undeployed workspace state. The release-policy statements are historical; the current governing policy is the maintainer-owned schema-2 review gate.
