...
login() {
  ...
  printf '%s' "$response" | jq --exit-status '.status == "ok"' >/dev/null
}
...
login 3000 /tmp/wiki-a.cookies
login 3001 /tmp/wiki-b.cookies

create_response=$(curl --fail --silent --show-error \
  --cookie /tmp/wiki-a.cookies \
  --header 'Content-Type: application/json' \
...
read_response=$(curl --fail --silent --show-error \
  --cookie /tmp/wiki-b.cookies \
  "http://127.0.0.1:3001/_api/pages/$page_id")
printf '%s' "$read_response" | jq --exit-status '.page.path == "multi-instance-smoke"' >/dev/null
...
start_wiki wiki-a 3000
wait_for_url http://127.0.0.1:3000/healthz
login 3000 /tmp/wiki-a-restarted.cookies
