...
postgres)
  ...
  docker run -d --name db --network=wiki-e2e -e "POSTGRES_PASSWORD=Password123!" -e "POSTGRES_USER=wiki" -e "POSTGRES_DB=wiki" "$POSTGRES_TEST_IMAGE"
  ...
  docker run -d -p 3000:3000 --name wiki --network=wiki-e2e -v wiki-data:/wiki/data -e "DB_TYPE=postgres" -e "DB_HOST=db" -e "DB_PORT=5432" -e "DB_NAME=wiki" -e "DB_USER=wiki" -e "DB_PASS=Password123!" "$WIKI_TEST_IMAGE"
  ;;
mysql)
  echo "Using MySQL..."
  docker run -d --name db --network=wiki-e2e -e "MYSQL_ROOT_PASSWORD=Password123!" -e "MYSQL_USER=wiki" -e "MYSQL_PASSWORD=Password123!" -e "MYSQL_DATABASE=wiki" "$MYSQL_TEST_IMAGE"
  ...
  docker run -d -p 3000:3000 --name wiki --network=wiki-e2e -v wiki-data:/wiki/data -e "DB_TYPE=mysql" -e "DB_HOST=db" -e "DB_PORT=3306" -e "DB_NAME=wiki" -e "DB_USER=wiki" -e "DB_PASS=Password123!" "$WIKI_TEST_IMAGE"
  ;;
mariadb)
  echo "Using MariaDB..."
  docker run -d --name db --network=wiki-e2e -e "MYSQL_ROOT_PASSWORD=Password123!" -e "MYSQL_USER=wiki" -e "MYSQL_PASSWORD=Password123!" -e "MYSQL_DATABASE=wiki" "$MARIADB_TEST_IMAGE"
  while ! docker exec db mysql --user=root --password=Password123! -e "SELECT 1" &> /dev/null ; do
    echo "Waiting for database connection..."
    sleep 2
  done
  docker exec db mariadb --user=root --password=Password123! --batch --skip-column-names -e "SELECT VERSION()"
  docker run -d -p 3000:3000 --name wiki --network=wiki-e2e -v wiki-data:/wiki/data -e "DB_TYPE=mariadb" -e "DB_HOST=db" -e "DB_PORT=3306" -e "DB_NAME=wiki" -e "DB_USER=wiki" -e "DB_PASS=Password123!" "$WIKI_TEST_IMAGE"
  ;;
mssql)
  echo "Using MS SQL Server..."
  docker run -d --name db --network=wiki-e2e -e "MSSQL_SA_PASSWORD=Password123!" -e "ACCEPT_EULA=Y" "$MSSQL_TEST_IMAGE"
  ...
  docker run -d -p 3000:3000 --name wiki --network=wiki-e2e -v wiki-data:/wiki/data -e "DB_TYPE=mssql" -e "DB_HOST=db" -e "DB_PORT=1433" -e "DB_NAME=wiki" -e "DB_USER=sa" -e "DB_PASS=Password123!" "$WIKI_TEST_IMAGE"
  ;;
sqlite)
  echo "Using SQLite..."
  docker run -d -p 3000:3000 --name wiki --network=wiki-e2e -e "DB_TYPE=sqlite" -e "DB_FILEPATH=/wiki/data/db.sqlite" -v wiki-data:/wiki/data "$WIKI_TEST_IMAGE"
  ;;
*)
