...
test('account and navigation remain useful through offline startup and reconnect', async ({ page, browserName }) => {
  ...
  await expect(page.locator('.offline-navigation a[href="/p/offline"]')).toHaveAttribute('href', '/p/offline')
...
test('offline Browse immediately lists saved copies and recovers without an online event', async ({ page, browserName }) => {
  test.skip(browserName === 'firefox', 'Playwright Firefox setOffline leaves network requests online.')
  ...
  page.on('request', request => { if (new URL(request.url()).pathname === '/_api/pages/tree') treeRequests.push(request.url()) })
  await page.context().setOffline(true)
  if ((page.viewportSize()?.width ?? 1280) < 960) await page.getByRole('button', { name: /open navigation/i }).click()
  ...
  const first = saved[0]!
  await navigation.locator(`a[href="${first.snapshot.canonicalPath}"]`).click()
...
test('missed disconnect switches Browse to saved pages and preserves the logo on first offline navigation', async ({ page, browserName }) => {
  ...
  await expect(page.locator('.nav-sidebar-loading-status')).toHaveCount(0)
  await expect(navigation.locator('.offline-navigation__pages a')).toHaveCount(1)
  await navigation.locator(`a[href="${saved.snapshot.canonicalPath}"]`).click()
  await expect(page.locator('.offline-application')).toBeVisible()
  await expect(page).toHaveURL(saved.snapshot.canonicalPath)
