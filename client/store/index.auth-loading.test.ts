...
describe('root authentication and loading ownership', () => {
  ...
  it('ignores a delayed whoami success after explicit logout', async () => {
    ...
    expect(store.offlineIdentityReady).toBe(false)
    releaseResponse(
      new Response(JSON.stringify({ authenticated: true, user: { id: 42, name: 'Late Alice' } }), {
        status: 200,
        headers: { 'content-type': 'application/json' }
      })
    )
  ...
  it('keeps A after a stale B response and accepts a later fresh A response', async () => {
    ...
    const fetch = vi.fn(async () => {
      attempt += 1
      if (attempt === 1) {
        return new Response(JSON.stringify({ authenticated: true, user: { id: 1, name: 'A-old' } }), {
          status: 200,
          headers: { 'content-type': 'application/json' }
        })
      }
      ...
      return new Response(JSON.stringify({ authenticated: true, user: { id: 1, name: 'A-new' } }), {
        status: 200,
        headers: { 'content-type': 'application/json' }
      })
    })
    ...
    expect(store.offlineIdentityReady).toBe(false)
    releaseB(
      new Response(JSON.stringify({ authenticated: true, user: { id: 2, name: 'B' } }), {
        status: 200,
        headers: { 'content-type': 'application/json' }
      })
    )
  ...
  it('purges ordinary drafts only for a confirmed account switch', async () => {
    ...
    const fetch = vi.fn(
      async () =>
        new Response(JSON.stringify({ authenticated: true, user: { id: 7, name: 'B' } }), {
          status: 200,
          headers: { 'content-type': 'application/json' }
        })
    )
