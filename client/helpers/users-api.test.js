...
describe('users api helper', () => {
  ...
  test('fetches and validates admin users list payloads', async () => {
    ...
    expect(
      await fetchAdminUsersList(fetchImpl, {
        page: 3,
        pageSize: 15,
        filter: 'ali',
        providerKey: 'local',
  ...
  test('surfaces API error messages for failed admin user creates', async () => {
    ...
    await expect(Promise.resolve(createAdminUser(fetchImpl, {}, 'Bad user create'))).rejects.toThrow(
      'You are not authorized to assign a user to a group with elevated permissions.'
    )
  })
