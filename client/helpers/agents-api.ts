...
const Session = z.object({ id: Uuid, title: z.string(), retention: z.enum(['temporary', 'saved']), status: z.enum(['active', 'deletion_pending']), executionMode: z.enum(['agent', 'generation-only']), version: z.number().int().positive(), providerProfileId: Uuid.nullable(), profileResolutionToken: z.string(), skills: z.array(Skill), currentRun: Run.nullable(), createdAt: Iso, updatedAt: Iso, lastActivityAt: Iso, expiresAt: Iso.nullable() })
...
const Profile = z.object({ id: Uuid, versionId: Uuid, name: z.string(), transport: z.enum(['openai-responses', 'openresponses', 'openai-chat', 'legacy-completions', 'anthropic-messages']), model: z.string(), destinationHost: z.string(), executionModes: z.array(z.enum(['agent', 'generation-only'])), capabilities: z.object({ streaming: z.boolean(), functions: z.boolean(), parallelFunctions: z.boolean(), structuredOutput: z.enum(['native-json-schema', 'tool-result', 'prompt-only']), usage: z.enum(['stream', 'terminal', 'estimated']), cancellation: z.boolean(), maxContextTokens: z.number(), maxOutputTokens: z.number() }), capabilityRevision: z.string(), policyVersion: z.number().int().positive(), isGlobalDefault: z.boolean(), storageDescription: z.string() })
...
export const updateAgentProfile = (fetcher: typeof fetch, csrfToken: string, sessionId: string, input: UpdateAgentSessionProfileRequest): Promise<AgentThreadState> =>
  requestJson(fetcher, csrfToken, `/_api/agents/sessions/${encodeURIComponent(sessionId)}/profile`, Thread, { method: 'PUT', body: JSON.stringify({ expectedSessionVersion: input.expectedSessionVersion, profileId: input.providerProfileId, executionMode: input.executionMode }) }) as Promise<AgentThreadState>
