...
function normalizeProviderConfig (row, fallbackMessage) {
...
function normalizeProvider (row, fallbackMessage) {
  ...
  return {
    ...
    logo: row.logo,
    website: row.website,
    config: row.config.map(cfg => normalizeProviderConfig(cfg, fallbackMessage)).sort((a, b) => {
...
function normalizeProvidersPayload (payload, fallbackMessage) {
...
async function fetchAnalyticsProviders (fetchImpl, fallbackMessage = 'Analytics providers response is invalid') {
