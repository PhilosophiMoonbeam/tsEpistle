...
describe('Markdown editor layout', () => {
  ...
  it('offers the same default-on follow toggle in both toolbars and follows cursor and rendered content', () => {
    ...
    for (const toolbar of [desktopToolbar, mobileToolbar]) {
      ...
      expect(toolbar).toContain("@click='togglePreviewAlignment'")
      expect(toolbar).toContain(":aria-pressed='previewAlignmentEnabled'")
      expect(toolbar).toContain(":variant='previewAlignmentEnabled ? `tonal` : `text`'")
      expect(toolbar).toContain("previewAlignmentEnabled ? 'Preview follows cursor' : 'Preview alignment off'")
      expect(toolbar.indexOf("@click='togglePreviewAlignment'")).toBeLessThan(toolbar.indexOf('previewShown = !previewShown'))
    }
    expect(template.match(/aria-label='Align preview to cursor'/g)).toHaveLength(2)
    expect(template.match(/@click='togglePreviewAlignment'/g)).toHaveLength(2)
    ...
    expect(script).toContain('previewAlignmentEnabled: true')
    expect(script).toMatch(/onCursor: position => \{\s*this\.positionSync\(position\)\s*this\.requestPreviewAlignment\(\)\s*\}/)
    expect(script).toMatch(/onClick: \(\) => \{\s*this\.requestPreviewAlignment\(\)\s*\}/)
    ...
    expect(processContent).toContain('this.requestPreviewAlignment(true)')
    expect(processContent.indexOf('this.previewDirty = false')).toBeGreaterThan(processContent.indexOf('enhanceWikiMarkdownPreview'))
    expect(processContent).toContain('revision !== this.previewRevision')

    const alignStart = script.indexOf('    alignPreviewToCursor (force = false)')
    const alignEnd = script.indexOf('    toggleHelp ()', alignStart)
    const alignMethod = script.slice(alignStart, alignEnd)
    expect(alignMethod).toContain('this.editorDisposed || !this.previewAlignmentEnabled || !this.previewShown || this.previewDirty || !this.cm || this.previewHTML.trim().length === 0')
    ...
    expect(script).toContain('previewAlignmentSchedulers.get(editor)?.cancel()')
    expect(script).toContain('previewAlignmentSchedulers.delete(this)')
    expect(script).toMatch(/if \(!this\.previewAlignmentEnabled\) \{\s*stopPreviewAlignment\(this\)/)
