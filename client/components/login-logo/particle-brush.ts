import {
  ...
  type LogoPointerImpulse,
  type LogoPointerState
} from './useLogoPointer'

const BRUSH_STRENGTH_GAIN = 1.3125
const BRUSH_RADIUS_GAIN = 1.12
...
export class ParticleBrush {
  ...
  update(time: number, pointer: LogoPointerState): void {
    ...
    for (const impulse of pointer.impulses) {
      if (!impulse.active || !Number.isFinite(impulse.ageSeconds) || impulse.ageSeconds >= LOGO_POINTER_IMPULSE_LIFETIME_SECONDS) continue
    ...
    const targetTravel = Math.min(
      LOGO_POINTER_MAX_TRAVEL_CSS,
      bounded(latest.travelCss, 0, LOGO_POINTER_MAX_SEGMENT_CSS) * bounded(latest.strength, 0.9, 3.2) * BRUSH_STRENGTH_GAIN
    ) * activity
    ...
    this.radius += (bounded(latest.radiusCss * BRUSH_RADIUS_GAIN, 18, LOGO_POINTER_MAX_RADIUS_CSS) - this.radius) * positionFollow
