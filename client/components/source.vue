  ...
  v-app().source
    ...
    v-main
      v-toolbar(color='primary')
        i18next.text-body-large(v-if='versionId > 0', path='common:page.viewingSourceVersion', tag='div')
          strong(place='date', :title='$helpers.formatMoment(versionDate, `LLL`)') {{ $helpers.formatMoment(versionDate, 'lll') }}
          strong(place='path') /{{path}}
        i18next.text-body-large(v-else, path='common:page.viewingSource', tag='div')
          strong(place='path') /{{path}}
        template(v-if='$vuetify.display.mdAndUp')
          v-spacer
          .text-body-small.text-blue-lighten-3 {{$t('common:page.id', { id: pageId })}}
          .text-body-small.text-blue-lighten-3.ml-4(v-if='versionId > 0') {{$t('common:page.versionId', { id: versionId })}}
          v-btn.ml-4(v-if='versionId > 0', variant="flat", color="blue-darken-1", @click='goHistory')
            v-icon mdi-history
          v-btn.ml-4(variant="flat", color="blue-darken-1", @click='goDownload')
...
.source {
  pre > code {
    ...
    @at-root .v-theme--dark.source pre > code {
