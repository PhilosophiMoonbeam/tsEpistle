...
describe('admin-dev-flags root UI facade migration guard', () => {
  ...
  test('admin-dev-flags.vue uses grouped root-ui-store facades for load and save root UI state', () => {
    ...
    expect(script).toMatch(/async\s+mounted\s*\(\s*\)\s*\{\s*loadingStart\s*\(\s*wikiStore\s*,\s*['"]admin-dev-flags-refresh['"]\s*\)[\s\S]*?try\s*\{[\s\S]*?this\.flags\s*=\s*await\s+fetchSystemFlags\s*\(\s*window\.fetch\.bind\(\s*window\s*\)\s*,\s*['"]System flags response is invalid['"]\s*\)[\s\S]*?this\.flagsLoaded\s*=\s*true[\s\S]*?\}\s*catch\s*\(\s*err\s*\)\s*\{\s*showNotification\s*\(\s*wikiStore\s*,\s*\{\s*style:\s*['"]red['"]\s*,\s*message:\s*getErrorMessage\s*\(\s*err\s*\)\s*,\s*icon:\s*['"]alert['"]\s*\}\s*\)\s*\}\s*loadingStop\s*\(\s*wikiStore\s*,\s*['"]admin-dev-flags-refresh['"]\s*\)/)

    expect(script).toMatch(/async\s+save\s*\(\s*\)\s*\{\s*if\s*\(\s*!this\.flagsLoaded\s*\)\s*\{\s*return\s*\}\s*loadingStart\s*\(\s*wikiStore\s*,\s*['"]admin-dev-flags-update['"]\s*\)[\s\S]*?try\s*\{[\s\S]*?await\s+updateSystemFlags\s*\(\s*window\.fetch\.bind\(\s*window\s*\)\s*,\s*this\.flags\s*,\s*['"]System flags update failed['"]\s*\)[\s\S]*?showNotification\s*\(\s*wikiStore\s*,\s*\{\s*style:\s*['"]success['"]\s*,\s*message:\s*['"]Flags applied successfully\.['"]\s*,\s*icon:\s*['"]check['"]\s*\}\s*\)[\s\S]*?\}\s*catch\s*\(\s*err\s*\)\s*\{\s*showNotification\s*\(\s*wikiStore\s*,\s*\{\s*style:\s*['"]red['"]\s*,\s*message:\s*getErrorMessage\s*\(\s*err\s*\)\s*,\s*icon:\s*['"]alert['"]\s*\}\s*\)\s*\}\s*loadingStop\s*\(\s*wikiStore\s*,\s*['"]admin-dev-flags-update['"]\s*\)/)
