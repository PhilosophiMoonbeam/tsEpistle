...
describe('admin-rendering root UI facade migration guard', () => {
  ...
  test('admin-rendering.vue uses typed store facades without changing renderer fetch flow', () => {
    ...
    expect(loadRenderers).toMatch(
      /catch\s*\(\s*err\s*\)\s*\{\s*this\.renderers\s*=\s*\[\]\s*this\.currentRenderer\s*=\s*createEmptyRenderer\s*\(\s*\)\s*this\.renderersLoaded\s*=\s*false\s*this\.renderersLoadError\s*=\s*true\s*if\s*\(\s*notifyError\s*\)\s*\{\s*showNotification\s*\(\s*wikiStore\s*,\s*\{\s*message:\s*getErrorMessage\s*\(\s*err\s*\)\s*,\s*style:\s*['"]red['"]\s*,\s*icon:\s*['"]alert['"]\s*\}\s*\)\s*\}\s*throw\s+err\s*\}/
    )
    expect(loadRenderers).toMatch(
  ...
  test('refresh waits for renderer reload before showing the success notification', () => {
    expect(refresh).toMatch(
      /async\s+refresh\s*\(\s*\)\s*\{\s*await\s+this\.loadRenderers\s*\(\s*\)\s*showNotification\s*\(\s*wikiStore\s*,\s*\{\s*message:\s*['"]Rendering active configuration has been reloaded\.['"]\s*,\s*style:\s*['"]success['"]\s*,\s*icon:\s*['"]cached['"]\s*\}\s*\)\s*\}/
    )
