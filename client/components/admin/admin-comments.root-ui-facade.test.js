...
describe('admin-comments root UI facade migration guard', () => {
  ...
  test('admin-comments.vue imports the root UI facades and keeps comment provider selection watchers intact', () => {
    ...
    expect(script).toMatch(/import\s+\{(?=[^}]*\bfetchCommentProviders\b)(?=[^}]*\bsaveCommentProviders\b)(?=[^}]*\btype\s+CommentProvider\b)[^}]*\}\s+from\s+['"]\.\.\/\.\.\/helpers\/comments-api['"]/)
    expect(script).not.toMatch(/graphql-tag|\bgql\b/)
    expect(watchBlock).toMatch(/selectedProvider\s*\(\s*newValue\s*:\s*string\s*\)\s*\{\s*this\.provider\s*=\s*_\.find\s*\(\s*this\.providers\s*,\s*\[\s*['"]key['"]\s*,\s*newValue\s*\]\s*\)\s*\|\|\s*\{\s*\}\s*\}/)
    expect(watchBlock).toMatch(/providers\s*\(\s*\)\s*\{\s*this\.selectedProvider\s*=\s*_\.get\s*\(\s*_\.find\s*\(\s*this\.providers\s*,\s*['"]isEnabled['"]\s*\)\s*,\s*['"]key['"]\s*,\s*['"]db['"]\s*\)\s*\}/)
  })
  ...
  test('refresh waits for provider reload before showing the success notification through the facade', () => {
    ...
    expect(refresh).toMatch(/async\s+refresh\s*\(\s*\)\s*\{\s*await\s+this\.loadProviders\s*\(\s*\)\s*showNotification\s*\(\s*wikiStore\s*,\s*\{\s*message:\s*this\.\$t\s*\(\s*['"]admin:comments\.listRefreshSuccess['"]\s*\)\s*,\s*style:\s*['"]success['"]\s*,\s*icon:\s*['"]cached['"]\s*\}\s*\)\s*\}/)
