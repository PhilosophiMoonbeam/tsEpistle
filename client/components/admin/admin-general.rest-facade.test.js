...
describe('admin-general site REST facade migration guard', () => {
  ...
  test('admin-general imports site REST and root UI facades instead of Apollo', () => {
  ...
  test('loadConfig fetches site config by REST with refresh loading and error facade', () => {
    ...
    expect(loadConfig).toMatch(/setLoading\s*\(\s*this\.\$store\s*,\s*['"]admin-site-refresh['"]\s*,\s*true\s*\)/)
    ...
    expect(loadConfig).toMatch(/setLoading\s*\(\s*this\.\$store\s*,\s*['"]admin-site-refresh['"]\s*,\s*false\s*\)/)
  })
  ...
  test('save preserves title validation, REST save payload, success notification, and root field updates', () => {
    ...
    expect(save).toMatch(/showNotification\s*\(\s*this\.\$store\s*,\s*\{\s*style:\s*['"]error['"][\s\S]*admin:general\.siteTitleInvalidChars[\s\S]*icon:\s*['"]alert['"]\s*\}/)
    ...
    expect(save).toMatch(/message:\s*this\.\$t\s*\(\s*['"]admin:general\.saveSuccess['"]\s*\)/)
    ...
    expect(save).toMatch(/pushGraphError\s*\(\s*this\.\$store\s*,\s*err\s*\)/)
    expect(save).toMatch(/loadingStop\s*\(\s*this\.\$store\s*,\s*['"]admin-site-update['"]\s*\)/)
  })
