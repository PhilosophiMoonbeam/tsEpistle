...
describe('admin utilities telemetry REST and root UI facade migration guard', () => {
  ...
  test('admin-utilities-telemetry.vue imports REST helpers and no telemetry GraphQL mutations', () => {
  ...
  test('loadTelemetry preserves refresh loading, REST fetch, notifyError, rethrow, and cleanup behavior', () => {
    ...
    expect(loadTelemetry).toMatch(/loadingStart\s*\(\s*this\.\$store\s*,\s*['"]admin-utilities-telemetry-refresh['"]\s*\)/)
    ...
    expect(loadTelemetry).toMatch(/if\s*\(\s*notifyError\s*\)\s*\{\s*pushGraphError\s*\(\s*this\.\$store\s*,\s*err\s*\)\s*\}\s*throw\s+err/)
    expect(loadTelemetry).toMatch(/finally\s*\{\s*loadingStop\s*\(\s*this\.\$store\s*,\s*['"]admin-utilities-telemetry-refresh['"]\s*\)\s*\}/)
  ...
  test('updateTelemetry uses REST helper while preserving loading, notification, and error behavior', () => {
    ...
    expect(updateTelemetry).toMatch(/loadingStart\s*\(\s*this\.\$store\s*,\s*['"]admin-utilities-telemetry-set['"]\s*\)/)
    expect(updateTelemetry).toMatch(/await\s+updateSystemTelemetry\s*\(\s*window\.fetch\.bind\(\s*window\s*\)\s*,\s*this\.telemetry\s*\)/)
    expect(updateTelemetry).toMatch(/showNotification\s*\(\s*this\.\$store\s*,\s*\{\s*message:\s*['"]Telemetry updated successfully\.['"]\s*,\s*style:\s*['"]success['"]\s*,\s*icon:\s*['"]check['"]\s*\}\s*\)/)
    expect(updateTelemetry).toMatch(/catch\s*\(\s*err\s*\)\s*\{\s*pushGraphError\s*\(\s*this\.\$store\s*,\s*err\s*\)\s*\}/)
    expect(updateTelemetry).toMatch(/finally\s*\{\s*loadingStop\s*\(\s*this\.\$store\s*,\s*['"]admin-utilities-telemetry-set['"]\s*\)\s*this\.loading\s*=\s*false\s*\}/)
  ...
  test('resetClientId uses REST helper while preserving silent reload, notification, and cleanup behavior', () => {
    ...
    expect(resetClientId).toMatch(/loadingStart\s*\(\s*this\.\$store\s*,\s*['"]admin-utilities-telemetry-resetid['"]\s*\)/)
    ...
    expect(resetClientId).toMatch(/showNotification\s*\(\s*this\.\$store\s*,\s*\{\s*message:\s*['"]Telemetry Client ID reset successfully\.['"]\s*,\s*style:\s*['"]success['"]\s*,\s*icon:\s*['"]check['"]\s*\}\s*\)/)
    expect(resetClientId).toMatch(/catch\s*\(\s*err\s*\)\s*\{\s*pushGraphError\s*\(\s*this\.\$store\s*,\s*err\s*\)\s*\}/)
    expect(resetClientId).toMatch(/finally\s*\{\s*loadingStop\s*\(\s*this\.\$store\s*,\s*['"]admin-utilities-telemetry-resetid['"]\s*\)\s*this\.loading\s*=\s*false\s*\}/)
