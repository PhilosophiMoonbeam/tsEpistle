...
describe('admin-api-create REST mutation migration guard', () => {
  ...
  test('generate() uses REST helper while preserving payload, generated-key, refresh, loading, and copy-dialog flow', () => {
    ...
    expect(generate).toMatch(/const\s+refreshed\s*=\s*this\.refreshApiKeys\s*\?\s*await\s+\(\s*this\.refreshApiKeys\s+as\s+\(\s*notify:\s*boolean\s*\)\s*=>\s*Promise<boolean>\s*\)\s*\(\s*false\s*\)\s*:\s*true/)
    expect(generate).toMatch(/this\.key\s*=\s*resp\.key/)
    expect(generate).toMatch(/this\.isCopyKeyDialogShown\s*=\s*true/)
    expect(generate).toMatch(/wikiStore\.showNotification\s*\(\s*\{[\s\S]*message:\s*this\.\$t\s*\(\s*['"]admin:api\.newKeySuccess['"]\s*\)[\s\S]*\}\s*\)/)
    expect(generate).toMatch(/wikiStore\.showError\s*\(\s*err\s*\)/)
    expect(generate).toMatch(/finally\s*\{[\s\S]*wikiStore\.stopLoading\s*\(\s*['"]admin-api-create['"]\s*\)/)
