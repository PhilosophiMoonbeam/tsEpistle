    ...
    v-row()
      v-col(cols='12')
        ...
        v-card.mt-3.animated.fadeInUp
          v-card-text
            ...
            v-tabs-window(v-model='contributeTab')
              ...
              v-tabs-window-item(value='tshirts', :transition='false', :reverse-transition='false')
                ...
                v-card-actions.ml-2
                  v-btn(outlined, :color='$vuetify.theme.current.dark ? `blue lighten-1` : `primary`', href='https://wikijs.threadless.com', large)
                    v-icon(left) mdi-tshirt-crew
                    span {{ $t('admin:contribute.shop') }}
            v-divider.mt-3
            v-list-subheader.subtitle-2  {{ $t('admin:contribute.contribute') }}
            .body-2.pl-3
              ul
            ...
            v-divider.mt-3
            v-list-subheader.subtitle-2  {{ $t('admin:contribute.spreadTheWord') }}
            .body-2.pl-3
              ul
          ...
          v-container.pa-5.grey(fluid, :class='$vuetify.theme.current.dark ? `darken-3` : `lighten-4`')
            ...
            v-row(dense)
              v-col(cols='12', lg='6', xl='4', v-for='(backer, idx) in backers', :key='backer.id')
                v-card.grey(flat, :class='$vuetify.theme.current.dark ? `darken-4` : `lighten-2`')
                  v-list-item
                    v-avatar
                    ...
                    div.v-list-item-content
                      ...
                      v-list-item-subtitle: .caption Since {{ $helpers.formatMoment(backer.joined, 'MMMM DD, YYYY') }} on {{backer.source}}
                    div.v-list-item-action(v-if='backer.twitter')
                      v-btn(icon, :href='backer.twitter', target='_blank')
                        v-icon(color='grey') mdi-twitter
                    div.v-list-item-action(v-if='backer.website')
