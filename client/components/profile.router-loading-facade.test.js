...
describe('profile router loading facade migration guard', () => {
  ...
  test('profile.vue router hooks use root-ui-store loading facades and preserve unrelated commits', () => {
    ...
    expect(script).toMatch(/import\s+store\s+from\s+['"]\.\.\/store['"]/)
    expect(script).toMatch(/import\s+\{(?=[^}]*\bloadingStart\b)(?=[^}]*\bloadingStop\b)[^}]*\}\s+from\s+['"]\.\.\/helpers\/root-ui-store['"]/)

    expect(script).toMatch(/router\.beforeEach\s*\(\s*\(\s*to\s*,\s*from\s*,\s*next\s*\)\s*=>\s*\{[\s\S]*?\bloadingStart\s*\(\s*store\s*,\s*['"]profile['"]\s*\)[\s\S]*?\bnext\s*\(\s*\)[\s\S]*?\}\s*\)/)
    expect(script).toMatch(/router\.afterEach\s*\(\s*\(\s*to\s*,\s*from\s*\)\s*=>\s*\{[\s\S]*?\bloadingStop\s*\(\s*store\s*,\s*['"]profile['"]\s*\)[\s\S]*?\}\s*\)/)
