...
describe('inline Ask mode contract', () => {
  ...
  test('uses one LIFO focus stack across Search, Ask, and compact panels', () => {
    ...
    expect(inline).toMatch(/const triggerForPanel = \(kind:[\s\S]*const root = inlineAgentRoot\.value[\s\S]*root\.querySelector<HTMLElement>[\s\S]*window\.matchMedia\(mobilePanelQuery\)\.matches/)
    expect(inline).toMatch(/const closeHistory = \(\): void => \{[\s\S]*const closingKind = compactPanels\.value && historyOpen\.value \? 'history' : null[\s\S]*preparePanelTriggerRestore\(closingKind\)[\s\S]*historyOpen\.value = false/)
    expect(inline).toMatch(/const closeMemory = \(\): void => \{[\s\S]*const closingKind = compactPanels\.value && memoryOpen\.value \? 'memory' : null[\s\S]*preparePanelTriggerRestore\(closingKind\)[\s\S]*memoryOpen\.value = false/)
    expect(inline).toMatch(/const updateMemoryOpen = \(open: boolean\): void => \{[\s\S]*else closeMemory\(\)/)
    expect(inline).toMatch(/const closePanels = \(\): void => \{[\s\S]*const closingKind = compactPanels\.value[\s\S]*preparePanelTriggerRestore\(closingKind\)[\s\S]*historyOpen\.value = false[\s\S]*memoryOpen\.value = false/)
    ...
    expect(inline).toMatch(/watch\(\[historyOpen, memoryOpen, compactPanels\], async \(\[history, memory, compact\]\) => \{[\s\S]*if \(!kind \|\| !compact\) \{\s*panelFocusScope\?\.deactivate\(\{ restoreFocus: false \}\)[\s\S]*return/)
    expect(inline).toMatch(/watch\(\[historyOpen, memoryOpen\],[\s\S]*const restoreKind = pendingPanelFocusKind[\s\S]*triggerForPanel\(restoreKind\)\?\.focus\(\{ preventScroll: true \}\)[\s\S]*\{ flush: 'post' \}/)
