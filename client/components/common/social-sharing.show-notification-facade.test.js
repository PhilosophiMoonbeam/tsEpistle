...
describe('social-sharing showNotification facade migration guard', () => {
  ...
  test('uses a semantic copy button and typed notifications around programmatic ClipboardJS', () => {
    ...
    expect(source).toMatch(/v-list-item\s*\(\s*tag=['"]button['"]\s*,\s*@click=['"]copyUrl['"]\s*\)/)
    expect(source).toMatch(
      /copyUrl\s*\(\s*\)\s*:\s*void\s*\{\s*try\s*\{\s*ClipboardJS\.copy\s*\(\s*this\.url\s*\)[\s\S]*?\bwikiStore\.showNotification\s*\(\s*\{\s*style:\s*['"]success['"]\s*,\s*message:\s*`URL copied successfully`\s*,\s*icon:\s*['"]content-copy['"]\s*\}\s*\)[\s\S]*?\}\s*catch\s*\{\s*wikiStore\.showNotification\s*\(\s*\{\s*style:\s*['"]red['"]\s*,\s*message:\s*`Failed to copy to clipboard`\s*,\s*icon:\s*['"]alert['"]\s*\}\s*\)[\s\S]*?\}\s*\}/
    )
