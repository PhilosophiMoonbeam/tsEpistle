<template>
    <v-card class="agent-memory" rounded="xl">
      ...
      <v-card-text class="agent-memory__body pa-5">
        ...
        <v-expand-transition>
          <v-sheet v-if="editing" class="agent-memory__editor pa-4 mb-5" border rounded="lg">
            ...
            <div class="d-flex justify-end ga-2">
              ...
              <v-btn color="primary" :disabled="!draftContent.trim()" :loading="saving" @click="save">Save memory</v-btn>
            </div>
          </v-sheet>
        </v-expand-transition>
        ...
        <section
          ...
          :key="section.target"
          class="agent-memory__section mb-4"
          :aria-labelledby="`agent-memory-${section.target}`"
        >
          <div class="d-flex align-center ga-3 mb-2">
            <v-avatar :color="section.color" size="34" variant="tonal">
              <v-icon :icon="section.icon" size="19" />
            </v-avatar>
            <div class="flex-grow-1">
          ...
          <v-progress-linear
            class="agent-memory__capacity mb-3"
            :color="section.store.characters / section.store.limit > .8 ? 'warning' : section.color"
            :model-value="section.store.characters / section.store.limit * 100"
            height="3"
            rounded
          />
          <div v-if="section.store.entries.length" class="agent-memory__entries">
            <div v-for="entry in section.store.entries" :key="entry.id" class="agent-memory__entry pa-3">
              ...
              <div class="agent-memory__entry-actions">
                <v-btn icon="mdi-pencil-outline" size="x-small" variant="text" :aria-label="`Edit memory: ${entry.content}`" @click="beginEdit(entry)" />
                <v-btn icon="mdi-delete-outline" size="x-small" variant="text" color="error" :aria-label="`Remove memory: ${entry.content}`" @click="removing = entry" />
              </div>
            </div>
          </div>
          <v-sheet v-else class="agent-memory__empty pa-4 text-body-medium text-medium-emphasis" rounded="lg">
            {{ section.empty }}
          </v-sheet>
        </section>

        <v-alert class="mt-5" color="info" icon="mdi-shield-lock-outline" variant="tonal">
          Memory is private to your Wiki account and bounded by design. Never store passwords, API keys, or short-lived details.
        </v-alert>
      </v-card-text>
  ...
  <v-dialog :model-value="Boolean(removing)" max-width="28rem" @update:model-value="value => { if (!value) removing = null }">
    <v-card rounded="xl" title="Remove this memory?">
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn variant="text" @click="removing = null">Cancel</v-btn>
  ...
  <v-dialog v-model="clearing" max-width="28rem">
    <v-card rounded="xl" title="Clear all Agent memory?">
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn variant="text" @click="clearing = false">Cancel</v-btn>
        <v-btn color="error" :loading="saving" @click="clear">Clear memory</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
const memoryCount = computed(() => memories.value.agent.entries.length + memories.value.user.entries.length)
const targetLimit = computed(() => draftTarget.value === 'user' ? 1_375 : 2_200)
...
const load = async (): Promise<void> => {
  ...
  try {
    ...
    error.value = message(value, 'Agent memory could not be loaded.')
  } finally {
    loading.value = false
  }
}
...
const remove = async (): Promise<void> => {
  ...
  error.value = ''
  try {
    await removeAgentMemory(window.fetch.bind(window), props.csrfToken, entry.id, entry.version)
    removing.value = null
    await load()
  } catch (value) {
    error.value = message(value, 'Memory could not be removed.')
  } finally {
    saving.value = false
  }
}
const clear = async (): Promise<void> => {
  ...
  try {
    ...
    error.value = message(value, 'Agent memory could not be cleared.')
  } finally {
    saving.value = false
  }
}
