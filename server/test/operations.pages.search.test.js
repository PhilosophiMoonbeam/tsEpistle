describe('page search visibility', () => {
  ...
  it('basic database search binds exact scope and escaped wildcard terms before its limit', async () => {
    const nestedBuilders = []
    const makeWhereBuilder = () => {
      const builder = {
        where: vi.fn().mockReturnThis(),
        orWhere: vi.fn().mockReturnThis(),
        andWhere: vi.fn(value => {
          if (typeof value === 'function') {
            const nested = makeWhereBuilder()
            nestedBuilders.push(nested)
            value(nested)
          }
          return builder
        })
    ...
    const rootWhere = makeWhereBuilder()
    const inScope = {
      id: 999,
      locale: 'en',
      path: 'ops%_/inside',
      title: 'Literal wildcard page',
      description: ''
    }
    const pageQuery = {
      column: vi.fn().mockReturnThis(),
      ...
      modifyGraph: vi.fn().mockReturnThis(),
      where: vi.fn(value => {
        if (typeof value === 'function') value(rootWhere)
        return pageQuery
      }),
      limit: vi.fn(async () => [inScope])
    }
    global.WIKI = {
      config: { db: { type: 'postgres' }, search: { maxHits: 7 } },
      models: { pages: { query: vi.fn().mockReturnValue(pageQuery) } }
    }
    const { default: engine } = await vi.importFresh('../modules/search/db/engine.ts', import.meta.url)

    const result = await engine.query('100%_', { locale: 'en', path: 'ops%_' })

    expect(result.results).toEqual([inScope])
    expect(rootWhere.andWhere).toHaveBeenCalledWith('localeCode', 'en')
    expect(nestedBuilders[0].where).toHaveBeenCalledWith('path', 'ops%_')
    expect(nestedBuilders[0].orWhere).toHaveBeenCalledWith('path', 'like', 'ops\\%\\_/%')
    expect(nestedBuilders[1].where).toHaveBeenCalledWith('title', 'ILIKE', '%100\\%\\_%')
    expect(nestedBuilders[1].orWhere).toHaveBeenCalledWith('description', 'ILIKE', '%100\\%\\_%')
    expect(pageQuery.limit).toHaveBeenCalledWith(7)
  })
})
