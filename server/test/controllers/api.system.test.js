...
describe('controllers/api system endpoints', () => {
  ...
  it('retires the old SSL status and unreviewed mutation routes without exposing or changing state', async () => {
    global.WIKI.auth.checkAccess.mockReturnValue(true)
    global.WIKI.servers.le = { requestCertificate: vi.fn() }
    const handlers = await loadHandlers()
    for (const name of ['ssl', 'updateSslRedirection', 'renewSslCertificate']) {
      const res = { set: vi.fn(), json: vi.fn(), sendStatus: vi.fn(), status: vi.fn().mockReturnThis() }
      await handlers[name]({ user: { permissions: ['manage:system'] }, body: { enabled: true } }, res)
      expect(res.status).toHaveBeenCalledWith(410)
      expect(res.set).toHaveBeenCalledWith('Cache-Control', 'no-store')
      expect(res.json.mock.calls[0][0].error).toContain('/_api/tls/workspace')
      expect(res.json.mock.calls[0][0]).not.toHaveProperty('sslStatus')
    }
    expect(global.WIKI.configSvc.saveToDb).not.toHaveBeenCalled()
