...
describe('agent proposal repository', () => {
  ...
  it('prepares a provenance-preserving OKF create and rejects stale replacement revisions', async () => {
    const missingPage = (): Error => Object.assign(new Error('missing'), { name: 'PageNotFoundError' })
    let existingPage: Record<string, unknown> | null = null
    const operations = {
      get: vi.fn(async () => {
        if (!existingPage) throw missingPage()
        return { ...existingPage }
      }),
      getByPath: vi.fn(async (input: Record<string, unknown>) => {
        if (input.visibility === 'private' || !existingPage) throw missingPage()
        return { ...existingPage }
      }),
      getVersion: vi.fn(),
    ...
    registerPageProposalActions(kernel, {
      ...
      operations,
      resolveRequester: async () => ({} as Express.User),
      resolveApprover: async () => ({} as Express.User),
      snapshotSigningSecret: Buffer.alloc(32, 7),
    ...
    const admissionSnapshot = {
      ...
      executionMode: 'agent' as const,
      supportsTools: true,
      permissions: ['use:mcp', 'write:pages'],
      groupIds: [3],
      featureFlags: flags
    }
    const authority = createActionAuthority(
      'pages.prepareImportOkf',
      '00000000-0000-4000-8000-000000000071',
      { kind: 'apiKey', apiKeyId: 9, groupId: 3, ownershipUserId: null, principal: null },
      admissionSnapshot
    )
    const document = `---
type: Reference
...
See [Next](./next.md).
`
    const prepared = await kernel.execute({
      authority,
      actionCallId: 'okf-create-call',
      input: { path: 'docs/imported', locale: 'en', document, expectedSourceRevision: null, isPublished: true },
      signal: new AbortController().signal,
      refreshAdmission: async () => admissionSnapshot
    }) as { proposalId: string; approvalId: string; status: string }
    expect(prepared).toMatchObject({ status: 'pending' })

    const stored = await knex('agentProposals').where({ actionName: 'pages.prepareImportOkf' }).first('operation')
    const operation = JSON.parse(String(stored.operation)) as Record<string, unknown>
    expect(operation).toMatchObject({
      ...
      operationInput: {
        title: 'Imported runbook',
        content: expect.stringContaining('[Next](/en/docs/next)'),
        okfMetadata: expect.objectContaining({
          producer_extension: { retained: true },
          generated: { by: 'process:tsfranki-import', at: '2026-08-18T12:00:00.000Z' }
        })
      },
      resultIdentity: expect.objectContaining({ okfSha256: expect.stringMatching(/^[a-f0-9]{64}$/u) })
    })
    ...
    await expect(kernel.execute({
      ...
      actionCallId: 'okf-apply-call',
      input: { proposalId: prepared.proposalId, approvalId: prepared.approvalId },
      signal: new AbortController().signal,
      refreshAdmission: async () => admissionSnapshot
    })).resolves.toMatchObject({
      status: 'applied',
      page: { id: 51, path: 'docs/imported', locale: 'en', sourceRevision: '1' }
    })
    expect(operations.create).toHaveBeenCalledOnce()
    expect(existingPage).toMatchObject({
      content: expect.stringContaining('[Next](/en/docs/next)'),
      extra: {
    ...
    await expect(kernel.execute({
      authority,
      actionCallId: 'okf-stale-replace-call',
      input: { path: 'docs/imported', locale: 'en', document, expectedSourceRevision: '7', isPublished: true },
      signal: new AbortController().signal,
      refreshAdmission: async () => admissionSnapshot
    })).rejects.toMatchObject({ code: 'PAGE_REVISION_CONFLICT', status: 409 })
  })
