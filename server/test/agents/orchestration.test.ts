...
const packet = (overrides: Readonly<Record<string, unknown>> = {}): string =>
  JSON.stringify({
    taskId: task.id,
    outcome: 'completed',
    claims: [
...
describe('agent orchestration contracts', () => {
  ...
  it('accepts only independent bounded task plans', () => {
    ...
    expect(
      parseAgentTaskPlan(
        JSON.stringify({ tasks: [planTask('Review alpha', 'What does alpha require?', 'alpha'), planTask('Review beta', 'What does beta require?', 'beta')] }),
        6
      )
    ).toHaveLength(2)
    ...
    expect(() =>
      parseAgentTaskPlan(
        JSON.stringify({
          tasks: [planTask('Review alpha', 'What does alpha require?', 'shared'), planTask('Review beta', 'What does beta require?', 'shared')]
        }),
        6
      )
    ).toThrow('overlapping')
  ...
  it('accepts only evidence read by the owning child at the exact source revision', () => {
    ...
    expect(() =>
      validateChildEvidencePacket(
        ...
        task,
        new Map([['page:42:section:1', 'rev-7']])
      )
  ...
  it('accounts child reservations by provider total tokens independently of directional counts', () => {
    const reservations = new AgentChildBudgetReservations(
      {
        ...DEFAULT_AGENT_ORCHESTRATION_LIMITS,
        childMaxOutputTokens: 6_000,
        maxAggregateChildTokens: 6_000
      },
