...
const createTables = async (db: Knex): Promise<void> => {
  ...
  await db.schema.createTable('agentMessages', table => {
    table.uuid('id').primary(); table.uuid('sessionId'); table.uuid('runId').nullable(); table.integer('ordinal'); table.string('role'); table.string('status'); table.text('content'); table.boolean('isVisible').notNullable().defaultTo(true); table.text('citations').nullable(); table.string('providerStateSha256').nullable(); table.dateTime('createdAt'); table.dateTime('updatedAt')
  })
  await db.schema.createTable('agentRuns', table => {
    table.uuid('id').primary(); table.uuid('sessionId'); table.uuid('userMessageId'); table.uuid('assistantMessageId'); table.uuid('clientRequestId'); table.uuid('goalId').nullable(); table.integer('goalContinuation').nullable(); table.string('status'); table.integer('attempts'); table.integer('maxAttempts'); table.integer('eventSequence'); table.dateTime('availableAt'); table.dateTime('cancelRequestedAt').nullable(); table.boolean('sideEffectsStarted'); table.uuid('providerProfileVersionId'); table.string('transportKind'); table.string('model'); table.string('executionMode'); table.integer('profilePolicyVersion'); table.integer('defaultGeneration'); table.string('capabilityRevision'); table.string('pricingRevision'); table.integer('promptVersion'); table.integer('inputTokens'); table.integer('outputTokens'); table.integer('estimatedCostMicros').nullable(); table.string('completionOutcome').nullable(); table.text('completionAssessment').nullable(); table.string('completionAssessmentSha256').nullable(); table.string('errorCode').nullable(); table.text('errorMessage').nullable(); table.dateTime('queuedAt'); table.dateTime('startedAt').nullable(); table.dateTime('updatedAt'); table.dateTime('completedAt').nullable()
  })
  await db.schema.createTable('agentGoals', table => {
    table.uuid('id').primary(); table.uuid('sessionId'); table.text('objective'); table.string('objectiveSha256'); table.string('status'); table.integer('version'); table.integer('continuationCount'); table.integer('maxContinuations'); table.bigInteger('consumedTokens'); table.bigInteger('maxTokens'); table.integer('consumedToolCalls'); table.integer('maxToolCalls'); table.string('completionOutcome').nullable(); table.text('completionAssessment').nullable(); table.string('completionAssessmentSha256').nullable(); table.string('errorCode').nullable(); table.text('errorMessage').nullable(); table.dateTime('startedAt'); table.dateTime('deadlineAt'); table.dateTime('updatedAt'); table.dateTime('completedAt').nullable()
  })
