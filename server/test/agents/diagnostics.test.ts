...
const createTables = async (db: Knex): Promise<void> => {
  ...
  await db.schema.createTable('agentGoals', table => {
    ...
    table.string('objectiveSha256')
    table.string('status')
    table.integer('version')
    table.integer('continuationCount')
    table.integer('maxContinuations')
    table.bigInteger('consumedTokens')
    table.bigInteger('maxTokens')
    table.integer('consumedToolCalls')
    table.integer('maxToolCalls')
    table.string('completionOutcome').nullable()
    table.text('completionAssessment').nullable()
    table.string('completionAssessmentSha256').nullable()
    table.string('errorCode').nullable()
...
interface DiagnosticUsageFixture {
...
const insertUsageRun = async (database: Knex, id: string, usage: DiagnosticUsageFixture): Promise<void> => {
  await database('agentRuns').insert({
    id,
    sessionId,
    userMessageId: null,
...
const expectDiagnosticFailure = async (database: Knex, code: string, message: string): Promise<void> => {
  await expect(Promise.resolve(exportAgentSessionDiagnostics(database, sessionId))).rejects.toMatchObject({ code, message, status: 500 })
}
...
describe('agent conversation diagnostics', () => {
  ...
  it('exports complete turns and derives evidence retries and duplicate historical page reads', async () => {
    ...
    const pageResult = JSON.stringify({
      id: 6,
    ...
    expect(exported.totals).toEqual({ inputTokens: 3, outputTokens: 309, totalTokens: 4_580, estimatedCostMicros: 0 })
    expect(exported.runs[0].usage).toEqual({ inputTokens: 3, outputTokens: 309, totalTokens: 4_580, estimatedCostMicros: 0 })
    expect(exported.messages).toEqual([
      ...
      expect.objectContaining({
        ...
        role: 'assistant',
        content: 'Amber Falcon.',
        providerContinuation: { sha256: 'a'.repeat(64), contentExported: false }
      })
    ])
    expect(exported.runs[0].diagnostics.toolCalls).toEqual([
      ...
      expect.objectContaining({
        actionCallId: 'get-2',
        inputRecorded: false,
        duplicateOfActionCallId: 'get-1',
        requestedAfterRejectedEvidenceDrafts: 2,
        requestReason: 'model_requested_after_evidence_rejection'
      })
    ])
  ...
  it('preserves nullable estimated cost semantics for valid zero usage', async () => {
    ...
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000201', {
      inputTokens: 0,
      outputTokens: 0,
      totalTokens: 0,
    ...
    const exported = (await exportAgentSessionDiagnostics(db, sessionId)) as unknown as DiagnosticExportView
    expect(exported.runs[0]?.usage).toEqual({ inputTokens: 0, outputTokens: 0, totalTokens: 0, estimatedCostMicros: null })
    expect(exported.totals).toEqual({ inputTokens: 0, outputTokens: 0, totalTokens: 0, estimatedCostMicros: 0 })
  })

  it('rejects individually valid rows whose aggregate input tokens overflow safely', async () => {
    db = await createUsageDatabase()
    const usage = { inputTokens: Number.MAX_SAFE_INTEGER, outputTokens: 0, totalTokens: Number.MAX_SAFE_INTEGER, estimatedCostMicros: null }
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000211', usage)
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000212', usage)

    await expectDiagnosticFailure(db, 'AGENT_DIAGNOSTIC_USAGE_OVERFLOW', 'Agent diagnostic usage totals exceed the supported numeric range')
  })

  it('rejects individually valid rows whose aggregate output tokens overflow safely', async () => {
    db = await createUsageDatabase()
    const usage = { inputTokens: 0, outputTokens: Number.MAX_SAFE_INTEGER, totalTokens: Number.MAX_SAFE_INTEGER, estimatedCostMicros: null }
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000221', usage)
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000222', usage)

    await expectDiagnosticFailure(db, 'AGENT_DIAGNOSTIC_USAGE_OVERFLOW', 'Agent diagnostic usage totals exceed the supported numeric range')
  })

  it('rejects individually valid rows whose aggregate independent totals overflow safely', async () => {
    db = await createUsageDatabase()
    const usage = { inputTokens: 0, outputTokens: 0, totalTokens: Number.MAX_SAFE_INTEGER, estimatedCostMicros: null }
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000231', usage)
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000232', usage)

    await expectDiagnosticFailure(db, 'AGENT_DIAGNOSTIC_USAGE_OVERFLOW', 'Agent diagnostic usage totals exceed the supported numeric range')
  })

  it('rejects individually valid rows whose aggregate estimated cost overflows safely', async () => {
    db = await createUsageDatabase()
    const usage = { inputTokens: 0, outputTokens: 0, totalTokens: 0, estimatedCostMicros: Number.MAX_SAFE_INTEGER }
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000241', usage)
    await insertUsageRun(db, '00000000-0000-4000-8000-000000000242', usage)

    await expectDiagnosticFailure(db, 'AGENT_DIAGNOSTIC_USAGE_OVERFLOW', 'Agent diagnostic usage totals exceed the supported numeric range')
  })

  it('rejects malformed or contradictory per-run usage as distinct corruption', async () => {
    db = await createUsageDatabase()
    const invalidRows: readonly { readonly id: string; readonly usage: DiagnosticUsageFixture }[] = [
      {
        id: '00000000-0000-4000-8000-000000000251',
        usage: { inputTokens: 3, outputTokens: 309, totalTokens: 311, estimatedCostMicros: null }
      },
      {
        id: '00000000-0000-4000-8000-000000000252',
        usage: { inputTokens: '3x', outputTokens: 309, totalTokens: 312, estimatedCostMicros: null }
      },
      {
        id: '00000000-0000-4000-8000-000000000253',
        usage: { inputTokens: 0, outputTokens: 0, totalTokens: 0, estimatedCostMicros: '0x' }
      }
    ]
    for (const row of invalidRows) {
      await insertUsageRun(db, row.id, row.usage)
      await expectDiagnosticFailure(db, 'AGENT_DIAGNOSTIC_USAGE_INVALID', 'Stored agent diagnostic usage is invalid')
      await db('agentRuns').where({ id: row.id }).delete()
    }
  })
})
