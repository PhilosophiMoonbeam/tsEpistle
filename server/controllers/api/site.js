...
const requirePlainObject = (value) => {
  if (!_.isPlainObject(value)) {
    throw new Error('Site configuration payload must be an object.')
  }
}

const applySiteConfigPatch = async (args) => {
...
router.put('/config', async (req, res) => {
  ...
  try {
    requirePlainObject(req.body)
    await applySiteConfigPatch(req.body)
    res.json({ message: 'Site configuration updated successfully' })
  } catch (err) {
    const status = /payload must be an object\.$/.test(err.message) ? 400 : 500
    res.status(status).json({ error: err.message || 'Site configuration update failed' })
  }
})
