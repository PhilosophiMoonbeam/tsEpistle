...
interface ManifestChunk extends Record<string, unknown> { file?: unknown, isEntry?: unknown }
interface CollectEntryOptions { dev?: boolean, origin?: string }
interface CollectedEntry { client: string | null, css: string[], file: string, preloads: string[] }
...
const collectEntry = (source: string, { dev = false, origin = 'http://127.0.0.1:5173' }: CollectEntryOptions = {}): CollectedEntry => {
  ...
  const css = new Set<string>(); const preloads = new Set<string>(); const visited = new Set<string>()
  const visit = (key: string): void => {
    ...
    if (Array.isArray(chunk.imports)) for (const importedKey of chunk.imports) {
