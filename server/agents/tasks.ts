...
const parsedPacket = (value: string | null, expectedSha256: string | null): AgentChildEvidencePacket | null => {
  ...
  if (expectedSha256 === null || createHash('sha256').update(value).digest('hex') !== expectedSha256)
    throw new AgentRepositoryError('AGENT_TASK_CORRUPT', 'Stored task result hash is invalid', 500)
  try {
...
const validTaskRow = (row: AgentTaskRow): AgentTaskRow => {
  ...
  if (
    ...
    row.ordinal < 0 ||
    !Number.isSafeInteger(row.requiredEvidenceCount) ||
    row.requiredEvidenceCount < 1 ||
    row.requiredEvidenceCount > 4 ||
...
const eventData = (
  row: Pick<AgentTaskRow, 'id' | 'runId' | 'subagentRunId' | 'kind' | 'title' | 'status' | 'outcome' | 'attempt' | 'evidenceCount' | 'errorCode'>
...
export const listAgentTaskViews = async (
  knex: Knex,
  ownerId: number,
  sessionId: string,
  ...
  const rows = await knex<AgentTaskRow>('agentRunTasks as tasks')
    ...
    .whereIn('tasks.runId', runIds)
    .select('tasks.*')
    .orderBy('runs.queuedAt')
    .orderBy('runs.id')
    .orderBy('tasks.ordinal')
    .orderBy('tasks.id')
    .limit(maximumTasks + 1)
  if (rows.length > maximumTasks) throw new AgentRepositoryError('AGENT_TASK_PROJECTION_OVERFLOW', 'Agent thread has too many tasks to project safely', 500)
  return rows.map(taskView)
}

export const createAgentRunTasks = async (
  knex: Knex,
  claim: AgentRunClaim,
  ...
  knex.transaction(async transaction => {
    ...
    for (const row of rows) {
      const failed = {
        ...
        subagentRunId: null,
        updatedAt: new Date(),
        errorCode: 'SUBAGENT_ATTEMPT_SUPERSEDED',
        errorMessage: 'The prior subagent attempt lost its root run lease'
      }
      await transaction('agentRunTasks')
        .where({ id: row.id, runId: claim.id, status: 'running', subagentRunId: row.subagentRunId })
        .update({ status: failed.status, subagentRunId: null, updatedAt: failed.updatedAt, errorCode: failed.errorCode, errorMessage: failed.errorMessage })
      await appendTaskEvent(transaction, claim, 'subagent.failed', { ...failed, subagentRunId: row.subagentRunId })
    }
  })

export const startAgentRunTask = async (knex: Knex, claim: AgentRunClaim, taskId: string, subagentRunId: string): Promise<AgentTaskRecord> =>
  knex.transaction(async transaction => {
    const row = await transaction<AgentTaskRow>('agentRunTasks').where({ id: taskId, runId: claim.id }).forUpdate().first()
    if (!row || row.status !== 'pending' || row.depth !== 1)
      throw new AgentRepositoryError('AGENT_TASK_STATE_CHANGED', 'Agent task cannot be started from its current state', 409)
    const now = new Date()
    const started: AgentTaskRow = {
      ...row,
      status: 'running',
      subagentRunId,
    ...
    const changed = await transaction('agentRunTasks').where({ id: taskId, runId: claim.id, status: 'pending', attempt: row.attempt }).update({
...
export const finishAgentRunTask = async (
  ...
  knex.transaction(async transaction => {
    ...
    const finished: AgentTaskRow = {
      ...row,
      status,
      outcome: validated.packet.outcome,
    ...
    const changed = await transaction('agentRunTasks').where({ id: taskId, runId: claim.id, status: 'running', subagentRunId }).update({
      status,
      outcome: finished.outcome,
      evidenceCount: finished.evidenceCount,
      authoritySha256,
      resultSha256,
      result: encoded,
      errorCode: null,
...
export const failAgentRunTask = async (knex: Knex, claim: AgentRunClaim, taskId: string, subagentRunId: string, errorCode: string): Promise<AgentTaskRecord> =>
  knex.transaction(async transaction => {
    const row = await transaction<AgentTaskRow>('agentRunTasks').where({ id: taskId, runId: claim.id }).forUpdate().first()
    if (!row || row.status !== 'running' || row.subagentRunId !== subagentRunId)
      throw new AgentRepositoryError('AGENT_TASK_STATE_CHANGED', 'Agent task failure does not own the active subagent attempt', 409)
    const now = new Date()
    const failed: AgentTaskRow = {
      ...row,
      status: 'failed',
      outcome: 'failed',
      errorCode,
      errorMessage: 'Subagent research failed',
      completedAt: now,
      updatedAt: now
    }
    await transaction('agentRunTasks')
      .where({ id: taskId, runId: claim.id, status: 'running', subagentRunId })
      .update({ status: failed.status, outcome: failed.outcome, errorCode, errorMessage: failed.errorMessage, completedAt: now, updatedAt: now })
...
export const cancelAgentRunTasks = async (
  knex: Knex,
  claim: AgentRunClaim,
  errorCode = 'RUN_CANCELLED',
  errorMessage = 'Root run was cancelled'
): Promise<void> =>
  knex.transaction(async transaction => {
    ...
    for (const row of rows) {
      ...
      await transaction('agentRunTasks')
        .where({ id: row.id, runId: claim.id })
        .whereIn('status', ['pending', 'running'])
        .update({ status: cancelled.status, errorCode: cancelled.errorCode, errorMessage: cancelled.errorMessage, completedAt: now, updatedAt: now })
