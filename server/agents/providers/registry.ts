...
const normalizedString = (value: string, label: string, maximum: number): string => {
  ...
  if (normalized.length < 1 || normalized.length > maximum || containsControlCharacter(normalized))
    throw new AgentRepositoryError('INVALID_PROVIDER_PROFILE', `${label} is invalid`, 400)
  return normalized
}
...
const normalizeBaseUrl = (value: string): string => {
  ...
  try {
    ...
    throw new AgentRepositoryError('INVALID_PROVIDER_URL', 'Provider base URL is invalid', 400)
  }
  ...
  if (
    ...
    throw new AgentRepositoryError('INVALID_PROVIDER_URL', 'Provider base URL must be a public HTTPS origin and base path', 400)
...
const validateHeaders = (headers: Readonly<Record<string, string>>): void => {
  for (const name of Object.keys(headers))
    if (FORBIDDEN_HEADERS.has(name.toLowerCase()) || name.toLowerCase().startsWith('x-forwarded-'))
      throw new AgentRepositoryError('INVALID_PROVIDER_HEADERS', `Provider header ${name} is not allowed`, 400)
}
...
const validateSettings = (input: AgentProviderSettingsInput, allowManagedReference = false) => {
  ...
  if (secretValue !== undefined && secretReference !== null)
    throw new AgentRepositoryError('INVALID_PROVIDER_SECRET', 'Provide either a credential or a secret reference, not both', 400)
  if (
    secretValue === undefined &&
    (secretReference === null ||
      (!EnvironmentSecretReference.test(secretReference) && !(allowManagedReference && ManagedSecretReference.test(secretReference))))
  )
    throw new AgentRepositoryError('INVALID_PROVIDER_SECRET', 'Secret reference must use env:NAME and must not contain the API key', 400)
  if ((transportKind === 'openai-responses' || transportKind === 'openresponses' || transportKind === 'openai-chat') && authMode !== 'bearer')
    throw new AgentRepositoryError('INVALID_PROVIDER_AUTH', 'Selected provider transport requires bearer authentication', 400)
  if (transportKind === 'anthropic-messages' && authMode !== 'anthropic-api-key')
    throw new AgentRepositoryError('INVALID_PROVIDER_AUTH', 'Anthropic Messages requires Anthropic API key authentication', 400)
  if (transportKind === 'gemini-api' && authMode !== 'google-api-key')
    throw new AgentRepositoryError('INVALID_PROVIDER_AUTH', 'Gemini API requires Google API key authentication', 400)
  ...
  if (
    ...
    (capabilities.toolCalling !== 'prompt' || capabilities.parallelToolCalls || capabilities.streaming || capabilities.structuredOutput !== 'prompt-only')
  )
    throw new AgentRepositoryError('INVALID_PROVIDER_CAPABILITIES', 'Legacy completions require buffered prompt tool calling and prompt-only output', 400)
  if (capabilities.toolCalling === 'prompt' && capabilities.parallelToolCalls)
    throw new AgentRepositoryError('INVALID_PROVIDER_CAPABILITIES', 'Prompt tool calling supports one action per model turn', 400)
  if (policies.allowedModes.includes('agent') && !capabilities.cancellation)
    throw new AgentRepositoryError('INVALID_PROVIDER_CAPABILITIES', 'Agent mode requires cancellation support', 400)
  const pricingRevision = normalizedString(input.pricingRevision, 'Pricing revision', 128)
  if (!AgentProviderPricingRevisionSchema.safeParse(pricingRevision).success) {
    throw new AgentRepositoryError('INVALID_PROVIDER_PRICING', 'Pricing revision must include immutable positive input and output token rates', 400)
  }
  return {
    transportKind,
    model,
...
const parseJson = <T>(schema: z.ZodType<T>, value: string, code: string): T => {
  try {
    ...
    throw new AgentRepositoryError(code, 'Stored provider profile data is invalid', 500)
  }
}
...
const supportsAgentExecution = (capabilities: AgentProviderCapabilities, policies: z.infer<typeof AgentProviderPoliciesSchema>): boolean =>
  policies.allowedModes.includes('agent') && capabilities.cancellation
...
export class AgentProviderRegistry implements AgentAdmissionResolver {
  ...
  async #configuration(transaction: Knex | Knex.Transaction): Promise<{ defaultGeneration: number }> {
    let row = (await transaction('agentProviderConfiguration').where({ id: 1 }).first('defaultGeneration')) as
        ...
        query
          ...
          .orWhereExists(
            database('agentProviderGrants')
              ...
              .whereRaw('"agentProviderGrants"."profileId" = "agentProviderProfiles"."id"')
  ...
  async create(input: CreateAgentProviderProfileInput): Promise<AgentProviderProfileView> {
    ...
    if (exposureMode === 'groups' && groupIds.length === 0)
    ...
    await this.#knex.transaction(async transaction => {
      ...
      await transaction('agentProviderProfileVersions').insert({
        ...
        model: value.model,
        utilityModel: value.utilityModel,
        baseUrl: value.baseUrl,
        authMode: value.authMode,
        secretReference,
        adapterConfig: canonicalJson(value.adapterConfig),
        capabilities: canonicalJson(value.capabilities),
        capabilityRevision: value.capabilityRevision,
        policies: canonicalJson(value.policies),
  ...
  async update(profileId: string, input: UpdateAgentProviderProfileInput): Promise<AgentProviderProfileAdminView> {
    ...
    await this.#knex.transaction(async transaction => {
      ...
      await transaction('agentProviderProfileVersions').insert({
        id: versionId,
        profileId,
        version: Number(currentSettings.version) + 1,
        transportKind: value.transportKind,
        model: value.model,
        utilityModel: value.utilityModel,
        baseUrl: value.baseUrl,
      ...
      await transaction('agentProviderProfiles')
        .where({ id: profileId, currentVersionId })
        .update({
          ...
          isGlobalDefault: false,
          conformed: false,
          policyVersion: Number(profile.policyVersion) + 1,
          updatedBy: input.actorId,
          updatedAt: now
        })
  ...
  async setConformed(profileId: string, versionId: string, conformed: boolean, actorId: number): Promise<void> {
    await this.#knex.transaction(async transaction => {
      ...
      await transaction('agentProviderProfiles')
        ...
        .update({
          conformed,
          ...(conformed ? {} : { status: 'disabled', isGlobalDefault: false }),
          policyVersion: Number(profile.policyVersion) + 1,
          updatedBy: actorId,
          updatedAt: new Date()
        })
  ...
  async setEnabled(profileId: string, enabled: boolean, actorId: number, expectedVersionId?: string): Promise<void> {
    await this.#knex.transaction(async transaction => {
      ...
      if (enabled && expectedVersionId === undefined)
        throw new AgentRepositoryError('PROFILE_VERSION_REQUIRED', 'Provider profile version is required when enabling', 409)
      if (expectedVersionId !== undefined && profile.currentVersionId !== expectedVersionId)
        throw new AgentRepositoryError('PROFILE_VERSION_CHANGED', 'Provider profile version changed', 409)
      const version = await transaction<VersionRow>('agentProviderProfileVersions').where({ id: profile.currentVersionId, profileId }).first()
      if (!version) throw new AgentRepositoryError('PROVIDER_PROFILE_CORRUPT', 'Current provider version is missing', 500)
      if (enabled && (!profile.conformed || !version.conformed || !version.secretReference || !(await this.#secrets.has(version.secretReference, transaction))))
        throw new AgentRepositoryError('PROFILE_NOT_READY', 'Provider profile is not conformed or its secret is unavailable', 409)
      const hasDefault =
        enabled && profile.exposureMode === 'all_agent_users'
          ? Boolean(
              await transaction('agentProviderProfiles')
                .where({ isGlobalDefault: true, status: 'enabled', conformed: true, exposureMode: 'all_agent_users' })
                .whereNull('deletedAt')
                .first('id')
            )
          : true
      ...
      await transaction('agentProviderProfiles')
        .where({ id: profileId, currentVersionId: profile.currentVersionId })
        .update({
          status: enabled ? 'enabled' : 'disabled',
          ...(enabled ? (becomesDefault ? { isGlobalDefault: true } : {}) : { isGlobalDefault: false }),
          policyVersion: Number(profile.policyVersion) + 1,
          updatedBy: actorId,
          updatedAt: new Date()
        })
      if ((!enabled && profile.isGlobalDefault) || becomesDefault)
  ...
  async setDefault(profileId: string, actorId: number): Promise<void> {
    await this.#knex.transaction(async transaction => {
      ...
      await transaction('agentProviderProfiles')
        ...
        .andWhere({ isGlobalDefault: true })
        .update({ isGlobalDefault: false, updatedBy: actorId, updatedAt: new Date() })
      await transaction('agentProviderProfiles')
        .where({ id: profileId })
        .update({ isGlobalDefault: true, policyVersion: Number(profile.policyVersion) + 1, updatedBy: actorId, updatedAt: new Date() })
      await transaction('agentProviderConfiguration')
        .where({ id: 1 })
        .update({ defaultGeneration: transaction.raw('?? + 1', ['defaultGeneration']), updatedBy: actorId, updatedAt: new Date() })
    })
  ...
  async remove(profileId: string, actorId: number): Promise<void> {
    await this.#knex.transaction(async transaction => {
      ...
      const references = await transaction<VersionRow>('agentProviderProfileVersions')
        .where({ profileId })
        .whereNotNull('secretReference')
        .pluck<string>('secretReference')
      const now = new Date()
      await transaction('agentProviderProfiles')
        .where({ id: profileId })
        .update({
      ...
      for (const reference of new Set(references)) await this.#revokeUnreferencedCredential(transaction, reference)
      if (profile.isGlobalDefault)
        await transaction('agentProviderConfiguration')
          .where({ id: 1 })
          .update({ defaultGeneration: transaction.raw('?? + 1', ['defaultGeneration']), updatedBy: actorId, updatedAt: now })
    })
  }

  async #getProfile(profileId: string): Promise<AgentProviderProfileAdminView> {
    const row = (await this.#knex<ProfileRow>('agentProviderProfiles')
      .where('agentProviderProfiles.id', profileId)
      .whereNull('agentProviderProfiles.deletedAt')
      .join('agentProviderProfileVersions', 'agentProviderProfileVersions.id', 'agentProviderProfiles.currentVersionId')
      .select(
        'agentProviderProfiles.*',
        'agentProviderProfileVersions.version',
        'agentProviderProfileVersions.transportKind',
        'agentProviderProfileVersions.model',
        'agentProviderProfileVersions.utilityModel',
        'agentProviderProfileVersions.baseUrl',
        'agentProviderProfileVersions.authMode',
        'agentProviderProfileVersions.secretReference',
        'agentProviderProfileVersions.adapterConfig',
        'agentProviderProfileVersions.capabilities',
        'agentProviderProfileVersions.capabilityRevision',
        'agentProviderProfileVersions.policies',
        'agentProviderProfileVersions.pricingRevision'
      )
      .first()) as (ProfileRow & VersionRow) | undefined
  ...
  async listAll(limit = 100): Promise<AgentProviderProfileAdminView[]> {
    const ids = await this.#knex('agentProviderProfiles')
      .whereNull('deletedAt')
      .orderBy('displayName')
      .limit(Math.max(1, Math.min(100, limit)))
      .pluck<string>('id')
  ...
  async setGrants(profileId: string, exposureMode: 'all_agent_users' | 'groups', groupIds: readonly number[], actorId: number): Promise<void> {
    ...
    await this.#knex.transaction(async transaction => {
      ...
      await transaction('agentProviderProfiles')
        .where({ id: profileId })
        .update({ exposureMode: mode, isGlobalDefault: false, policyVersion: Number(profile.policyVersion) + 1, updatedBy: actorId, updatedAt: new Date() })
      if (profile.isGlobalDefault)
        await transaction('agentProviderConfiguration')
          .where({ id: 1 })
          .update({ defaultGeneration: transaction.raw('?? + 1', ['defaultGeneration']), updatedBy: actorId, updatedAt: new Date() })
  ...
  async setSessionProfile(input: {
    ...
    await this.#knex.transaction(async transaction => {
      ...
      const profile = resolvedProfileId
        ? await transaction<ProfileRow>('agentProviderProfiles')
            .where({ id: resolvedProfileId, status: 'enabled', conformed: true })
            .whereNull('deletedAt')
            .first()
        : undefined
      ...
      if (!version?.secretReference || !allowed || !(await this.#secrets.has(version.secretReference, transaction)))
    ...
    const payload = await this.#knex.transaction(async transaction => {
      ...
      if (!profile?.currentVersionId)
        throw new AgentRepositoryError(
          'PROFILE_UNAVAILABLE',
          session.providerProfileId ? 'Selected provider profile is unavailable' : 'No default provider profile is configured for your groups.',
          409
        )
      const version = await transaction<VersionRow>('agentProviderProfileVersions')
        .where({ id: profile.currentVersionId, profileId: profile.id, conformed: true })
        .first()
      if (!version) throw new AgentRepositoryError('PROVIDER_PROFILE_CORRUPT', 'Current provider version is missing', 500)
      const capabilities = parseJson(AgentProviderCapabilitiesSchema, version.capabilities, 'PROVIDER_PROFILE_CORRUPT')
      const policies = parseJson(AgentProviderPoliciesSchema, version.policies, 'PROVIDER_PROFILE_CORRUPT')
  ...
  async resolve(input: { readonly ownerId: number; readonly sessionId: string; readonly profileResolutionToken: string }): Promise<AgentResolvedAdmission> {
    ...
    return this.#knex.transaction(async transaction => {
      ...
      if (!allowed || !version.secretReference || !(await this.#secrets.has(version.secretReference, transaction)))
        throw new AgentRepositoryError('PROFILE_UNAVAILABLE', 'Provider profile is not available to this user', 409)
      const capabilities = parseJson(AgentProviderCapabilitiesSchema, version.capabilities, 'PROVIDER_PROFILE_CORRUPT')
      const policies = parseJson(AgentProviderPoliciesSchema, version.policies, 'PROVIDER_PROFILE_CORRUPT')
      if (!supportsAgentExecution(capabilities, policies))
        throw new AgentRepositoryError('PROFILE_MODE_INCOMPATIBLE', 'Provider profile does not support Wiki Agent actions', 409)
