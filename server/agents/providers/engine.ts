...
Retrieval. Read an exact current page or historical version directly with the admitted ${AGENT_TOOL_NAMES['pages.get']} or ${AGENT_TOOL_NAMES['pages.getVersion']} action when the request supplies its accepted identity; honor selector requirements, preserve the version, and never infer IDs. For a basic recent recap, call ${AGENT_TOOL_NAMES['pages.listRecent']} once with limit 10 instead of searching or reading each page; cite every returned row, and identify bounded opening excerpts when truncated. For explicit tag-taxonomy or tag/path/lifecycle browsing, use the applicable ${AGENT_TOOL_NAMES['pages.searchTags']}, ${AGENT_TOOL_NAMES['pages.listTags']}, or ${AGENT_TOOL_NAMES['pages.discover']} action directly. Otherwise, make one targeted ${AGENT_TOOL_NAMES['pages.search']} for the distinctive subject and requested facets, then read promising relevant candidates; broad inventories also require inspecting plausible differently titled candidates. Do not make a broad or synonym follow-up search once requested facets are supported. Search again only for a specific missing facet, weak match, ambiguity, conflict, temporal need, or useful reported continuation—not merely for more results. Candidate novelty, score, trust, and knowledge hints guide inspection, not relevance or authority; apply locale, path, lifecycle, trust, staleness, or concept-type filters when useful. ${AGENT_TOOL_NAMES['pages.related']} follows explicit links; continue with nextCursor only when useful.
...
const SUMMARY_INSTRUCTIONS = `When summarizing a page, cover its substantive key sections with concise, source-faithful points, not only a title, inventory, or isolated quotation. Include concrete details from each substantive section when present (such as a condition, date, qualifier, or member's description), not only headings, link labels, or brand names; do not invent details for navigation-only sections. Use real Markdown headings separated from points by blank lines.
