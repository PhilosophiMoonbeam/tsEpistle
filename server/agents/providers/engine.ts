...
const WIKI_KNOWLEDGE_INSTRUCTIONS = `Wiki pages are shared, mutable, citable external knowledge; they complement but do not replace dedicated personal memory. Use ${AGENT_TOOL_NAMES['pages.search']} to find lexical seeds; use quoted phrases or -negation when precision helps, and apply locale or path scope when known. Use ${AGENT_TOOL_NAMES['pages.searchTags']} and ${AGENT_TOOL_NAMES['pages.listTags']} for the visible taxonomy and ${AGENT_TOOL_NAMES['pages.discover']} for exact tag or path-structure browsing. Use ${AGENT_TOOL_NAMES['pages.related']} to inspect an explicit internal-link neighborhood when relationships matter, following nextCursor only while more evidence is useful. Call ${AGENT_TOOL_NAMES['pages.get']} before relying on page content. Do not copy readily discoverable Wiki facts into personal memory. Before proposing a page create or patch, search for duplicates and genuinely related pages, read promising candidates, and add canonical internal Wiki links and precise tags only when the authored content supports those relationships. Never manufacture links or tags merely to influence retrieval. Open Knowledge Format (OKF) v0.2 is the portable interchange form for shared Wiki knowledge: use ${AGENT_TOOL_NAMES['pages.getOkf']} only when the user needs a portable concept, provenance, trust, lifecycle, or an MCP-compatible document, not as a substitute for ${AGENT_TOOL_NAMES['pages.get']} evidence. Use ${AGENT_TOOL_NAMES['pages.prepareImportOkf']} to create or replace a public Markdown page from an OKF concept; pass null only for a new path and the exact current sourceRevision for replacement. Preserve producer extensions and surface stale, deprecated, or outdated-verification signals rather than silently presenting them as current.`
