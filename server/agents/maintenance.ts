...
const recoverRuns = async (knex: Knex, now: Date, batchSize: number): Promise<{ cancelled: number, recovered: number, requeued: number }> => knex.transaction(async transaction => {
  const rows = await transaction('agentRuns')
    .where(query => query.whereNotNull('cancelRequestedAt').whereIn('status', ['queued', 'running', 'awaiting_approval']).orWhere(subquery => subquery.whereIn('status', ['running', 'awaiting_approval']).andWhere('leaseExpiresAt', '<=', now)))
    .orderBy('updatedAt')
    .limit(batchSize)
  ...
  for (const row of rows) {
    if (row.cancelRequestedAt !== null) {
      cancelled += await transaction('agentRuns').where({ id: row.id, status: row.status }).update({ status: 'cancelled', completedAt: now, updatedAt: now, leaseOwner: null, leaseToken: null, leaseExpiresAt: null })
    } else if (row.status === 'running' && row.sideEffectsStarted) {
      recovered += await transaction('agentRuns').where({ id: row.id, status: 'running' }).update({ status: 'recovery_required', completedAt: now, updatedAt: now, leaseOwner: null, leaseToken: null, leaseExpiresAt: null, errorCode: 'LEASE_LOST_AFTER_SIDE_EFFECT', errorMessage: 'Run lease expired after a side effect may have started' })
    } else if (row.status === 'running') {
      requeued += await transaction('agentRuns').where({ id: row.id, status: 'running' }).update({ status: 'queued', availableAt: now, updatedAt: now, leaseOwner: null, leaseToken: null, leaseExpiresAt: null })
    } else {
      await transaction('agentRuns').where({ id: row.id, status: 'awaiting_approval' }).update({ leaseOwner: null, leaseToken: null, leaseExpiresAt: now, updatedAt: now })
    }
  }
...
const scrubMcpProposals = async (knex: Knex, cutoff: Date, now: Date, batchSize: number): Promise<number> => knex.transaction(async transaction => {
  const ids = await transaction('agentProposals').where({ sourceKind: 'mcp' }).whereIn('status', TERMINAL_PROPOSAL_STATUSES).whereNull('contentPurgedAt').andWhere('createdAt', '<=', cutoff).orderBy('createdAt').limit(batchSize).forUpdate().pluck<string>('id')
  if (ids.length === 0) return 0
...
const purgeMcpProposals = async (knex: Knex, cutoff: Date, batchSize: number): Promise<number> => {
  const ids = await knex('agentProposals').where({ sourceKind: 'mcp' }).whereIn('status', TERMINAL_PROPOSAL_STATUSES).andWhere('createdAt', '<=', cutoff).orderBy('createdAt').limit(batchSize).pluck<string>('id')
  if (ids.length === 0) return 0
...
const scrubSkillUses = async (knex: Knex, cutoff: Date, batchSize: number): Promise<number> => {
  const ids = await knex('agentSkillUses').whereNull('sessionId').whereNull('runId').whereNotNull('requesterApiKeyId').whereNotNull('resourcePath').andWhere('createdAt', '<=', cutoff).orderBy('createdAt').limit(batchSize).pluck<string>('id')
  if (ids.length === 0) return 0
...
const purgeSkillUses = async (knex: Knex, cutoff: Date, batchSize: number): Promise<number> => {
  const ids = await knex('agentSkillUses').whereNull('sessionId').whereNull('runId').whereNotNull('requesterApiKeyId').andWhere('createdAt', '<=', cutoff).orderBy('createdAt').limit(batchSize).pluck<string>('id')
  if (ids.length === 0) return 0
...
const compactEvents = async (knex: Knex, cutoff: Date, batchSize: number): Promise<number> => {
  const rows = await knex('agentEvents').join('agentRuns', 'agentRuns.id', 'agentEvents.runId').where('agentEvents.type', 'message.delta').whereIn('agentRuns.status', TERMINAL_RUN_STATUSES).andWhere('agentEvents.createdAt', '<=', cutoff).andWhereNot('agentEvents.data', canonicalJson({ compacted: true })).orderBy('agentEvents.createdAt').limit(batchSize).select<{ id: string }[]>('agentEvents.id')
  const ids = rows.map(row => row.id)
  if (ids.length === 0) return 0
  const data = canonicalJson({ compacted: true })
  return knex('agentEvents').whereIn('id', ids).update({ data, dataSha256: sha256(data) })
}

const reconcileExpiredReservations = async (knex: Knex, now: Date, batchSize: number): Promise<number> => {
  const rows = await knex('agentQuotaReservations').where({ status: 'reserved' }).andWhere('expiresAt', '<=', now).orderBy('expiresAt').limit(batchSize).select<{ runId: string, ownerId: number }[]>('runId', 'ownerId')
  let reconciled = 0
  for (const row of rows) {
