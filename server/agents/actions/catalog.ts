...
export const ACTION_CATALOG = {
  'pages.search': {
    descriptor: descriptor(
      'pages.search',
      'Search pages',
      `Rank visible pages by source authority, knowledge projections, utility-enriched declared gaps, and the link graph. Natural-language queries work; lifecycle filters constrain trust and maintenance state. Results carry match evidence and spelling suggestions but are candidate metadata only, not read evidence or authorization. After results, read each chosen page with ${AGENT_TOOL_NAMES['pages.get']}({id: result.id}), copying the positive numeric ID exactly. Never guess, stringify, or substitute a result's path/locale, href, citation/evidenceId, okfResourceUri, sourceRevision, title, or history versionId. Use a path/locale selector only for a user-supplied raw stored path and separate locale. The direct read reauthorizes current content. If a read is reported omitted or not_executed, disclose the gap and synthesize only from delivered reads; do not retry automatically or infer access.`,
      'read',
      ['read:pages'],
      both,
      readAnnotations
  ...
  'pages.discover': {
    descriptor: descriptor(
      'pages.discover',
      'Discover pages',
      `Browse visible pages by locale, descendant path depth, exact tags, stable ordering, and projected lifecycle or trust state. Knowledge filters run over a bounded 100-page candidate window; narrow the path if it is too broad. Results are candidate metadata only, not read evidence or authorization. After results, read each chosen page with ${AGENT_TOOL_NAMES['pages.get']}({id: result.id}), copying the positive numeric ID exactly. Never guess, stringify, or substitute a result's path/locale, href, citation/evidenceId, okfResourceUri, sourceRevision, title, or history versionId. Use a path/locale selector only for a user-supplied raw stored path and separate locale. The direct read reauthorizes current content. If a read is reported omitted or not_executed, disclose the gap and synthesize only from delivered reads; do not retry automatically or infer access.`,
      'read',
      ['read:pages'],
      both,
      readAnnotations
  ...
  'pages.get': {
    descriptor: descriptor(
      'pages.get',
      'Get page',
      `Read one visible Wiki page by an exact positive numeric ID or by a user-supplied raw stored path and separate locale. When following a search, discovery, recent, or related result, call ${AGENT_TOOL_NAMES['pages.get']}({id: result.id}) with the returned positive numeric result.id copied exactly. Never guess, stringify, or substitute the result's path/locale, href, citation/evidenceId, okfResourceUri, sourceRevision, title, or history versionId; path/locale is only for user-supplied path lookup and is not identity-preserving when copied from listed metadata. The direct read reauthorizes current content; candidate visibility and metadata grant no durable access and are not evidence. If a read is reported omitted or not_executed, disclose the gap and synthesize only from delivered reads; do not retry automatically or infer access.`,
      'read',
      ['read:pages'],
      both,
      readAnnotations
