...
export class AgentMediaUploadGate {
  ...
  async run<T>(ownerId: number, operation: () => Promise<T>): Promise<T> {
    ...
    if (this.#active >= 1 || ownerActive >= 1)
...
export const parseAgentMediaUpload = (parse: RequestHandler, req: Request, res: Response, signal: AbortSignal): Promise<void> =>
  new Promise<void>((resolve, reject) => {
    ...
    try {
      parse(req, res, error =>
        complete(error ? new AgentRepositoryError('INVALID_AGENT_MEDIA', 'Choose one PDF up to 250 MB, or an image or recording up to 10 MB.', 400) : undefined)
      )
    } catch (error) {
      complete(error)
    }
