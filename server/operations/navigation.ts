...
const validItem = (item: unknown): item is NavigationItem => {
  if (!item || !_.isPlainObject(item) || !_.isString(Reflect.get(item, 'id')) || Reflect.get(item, 'id').length < 1 || !_.isString(Reflect.get(item, 'kind')) || Reflect.get(item, 'kind').length < 1) return false
...
const validTree = (tree: unknown): tree is NavigationRow[] => Array.isArray(tree) && tree.every(row =>
  row && _.isPlainObject(row) && _.isString(Reflect.get(row, 'locale')) && Reflect.get(row, 'locale').length > 0 &&
  Array.isArray(Reflect.get(row, 'items')) && Reflect.get(row, 'items').every(validItem)
)

const serializeItem = (item: NavigationItem): Partial<NavigationItem> => _.pick(item, ['id', 'kind', 'label', 'icon', 'targetType', 'target', 'visibilityMode', 'visibilityGroups'])
