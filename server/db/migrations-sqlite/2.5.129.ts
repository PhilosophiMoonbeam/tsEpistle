...
const privateCount = async (knex: Knex, table: string): Promise<number> => {
  const row = await knex(table).where('isPrivate', true).count<{ count: number }[]>({ count: '*' }).first()
...
export const down = async (knex: Knex) => {
  ...
  await knex.raw(`UPDATE pages SET isPrivate = (visibility = 'private'), privateNS = CASE WHEN visibility = 'private' THEN 'owner:' || ownerId ELSE NULL END`)
  await knex.raw(`UPDATE pageHistory SET isPrivate = (visibility = 'private'), privateNS = CASE WHEN visibility = 'private' THEN 'owner:' || ownerId ELSE NULL END`)
  await knex.raw(`UPDATE pageTree SET isPrivate = (visibility = 'private'), privateNS = CASE WHEN visibility = 'private' THEN 'owner:' || ownerId ELSE NULL END`)
  await knex.schema.alterTable('pageTree', table => {
