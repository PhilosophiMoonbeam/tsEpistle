...
interface WorkerWiki {
  IS_DEBUG: boolean
  ROOTPATH: string
  SERVERPATH: string
  Error: typeof errorTypes
  configSvc: typeof configService
  CONNECTION_ROLE: 'worker'
  logger?: ReturnType<typeof loggerService.init>
  [key: string]: unknown
}
const hideBin = (argv: readonly string[]): string[] => {
  const isBundledElectronApp = Boolean(process.versions.electron) && !('defaultApp' in process && process.defaultApp)
  return argv.slice(isBundledElectronApp ? 1 : 2)
}
const workerWiki: WorkerWiki = {
